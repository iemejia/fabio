...
- **Endorsement / certification is READ-ONLY (there is NO public SET API)**: `fabio item show-endorsement --id <id> [--type <T>]` and `item list-endorsements [--endorsement Certified|Promoted|None] [--type <T>]` surface an item's endorsement (`{endorsement, certifiedBy, certificationDetails}`) for governance/CI checks. **Setting/certifying an item cannot be done via any public REST API** — this was verified exhaustively: the entire Fabric REST API spec (`microsoft/fabric-rest-api-specs`, all 54 workload swaggers + common + admin) contains ZERO endorsement operations; Fabric `Update Item` rejects an `endorsementDetails` body with HTTP 400; and the Power BI `Datasets` operation list has no "Set Endorsement". Endorsement is portal-only (the "Certification" tenant setting `CertifyDatasets` only governs WHO may certify in the UI — it exposes no API). Endorsement IS readable via the **Power BI REST API** (`GET /v1.0/myorg/groups/{ws}/{datasets|reports|dataflows|dashboards|datamarts}` returns `endorsementDetails`, null when unset → reported as `"None"`), so the read commands are scoped to the Power BI-lineage types the API exposes it for: `SemanticModel`, `Report`, `PaginatedReport`, `Dashboard`, `Dataflow`, `Datamart` (Fabric-native items like Lakehouse/Notebook return a clear "not readable via the API for type X" error — their endorsement is only in the admin metadata scanner). `show-endorsement` auto-detects the type from the Fabric item API when `--type` is omitted, then reads the matching Power BI collection. Pure helpers (`pbi_collection_for_type`, `parse_endorsement`, `normalize_endorsement_filter`, `row_type_label`, `item_id`) in `src/commands/item/endorsement.rs` are unit-tested; the read/filter/error paths are live-validated (`item_endorsement_read_lifecycle`). Both commands are `mutates:false` (read-only).
...
- **Git Workspace Relations (Preview)**: New `fabio git relation list|create|delete` commands implement the Fabric REST `WorkspaceRelations` API (`GET/POST /workspaces/{id}/git/workspaceRelations`, `DELETE .../{relationId}`) for managing base/branch links between workspaces as an independent resource (previously only implicit via `git branch-out`). Implemented in `src/commands/git/relation.rs` (the `git` command is a directory module — `mod.rs` enum/dispatch + `sync.rs` + `connect.rs` + `branch_out.rs` + `relation.rs`). See `.agents/API-BEHAVIORS-DISCOVERED.md` "Git Workspace Relations API Behaviors Discovered (Preview)" for API semantics and error codes.
 ...
 - **Ontology granular element editing (`add/delete-entity-type`, `add/delete-relationship-type`, `add/list/delete-report-link`)**: The Fabric ontology REST API is definition-based (CRUD + `getDefinition`/`updateDefinition`; NO per-element endpoints), so these 7 commands (in `src/commands/ontology/elements.rs`) implement client-side **read-modify-write**: fetch the definition parts, add/remove/edit exactly one element, push the whole set back via `updateDefinition` (LRO). Part shapes match `import`/`generate` — `EntityTypes/{id}/definition.json` (`namespaceType: "Custom"`, `visibility: "Visible"`, `properties[]` with `valueType`, `entityIdParts` for keys), `RelationshipTypes/{id}/definition.json` (`source`/`target.entityTypeId`), `EntityTypes/{id}/ResourceLinks/definition.json` (`{type: "PowerBIReport", workspaceId, itemId}`). `add-entity-type --name --property NAME:TYPE (repeatable) --key PROPNAME` (types normalized: String/Long/BigInt/Double/Decimal/Boolean/DateTime/Any + aliases); `add-relationship-type --name --source --target` (entity refs resolve by name OR id); `add-report-link --entity --report-workspace --report` (Power BI report `ResourceLink`); `list-report-links [--entity]`; the three `delete-*` are `destructive:true`, dry-run-guarded, and `delete-entity-type` **cascades** relationship types referencing the removed entity (reported as `cascadedRelationshipTypes`, previewable via `--dry-run`). New numeric element ids are collision-checked against every existing id (`collect_ids` + `gen_unique_id`). NOTE these edit the *definition* only — they do NOT provision Fabric IQ reasoning (still portal-exclusive, see the `ontology search` gates). Pure helpers (`normalize_value_type`, `parse_property_spec`, `build_entity_type_def`/`build_relationship_type_def`/`build_report_link`, `resolve_entity_id`, `collect_ids`/`gen_unique_id`, `encode_part`/`part_json`) are unit-tested (9 tests); the full add→verify→delete lifecycle (incl. the relationship cascade) is live-validated in `ontology_granular_elements_lifecycle`.
...
ghcr.io/iemejia/fabio:0.55.0       # release version
ghcr.io/iemejia/fabio:0.55         # major.minor
```
