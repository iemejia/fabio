...
| (consistency invariant — no file) | every CLI group ↔ a skill family / persona / cross-cutting allowlist | `every_command_group_has_a_knowledge_home` | New command GROUP added — give it a `data/skills/<family>.json` `command_groups` entry or a persona `delegates_to[].command_groups` entry |
...
`every_command_group_has_a_knowledge_home` (in `skillgen.rs`) is a third consistency gate: it fails if any command group is missing from BOTH the skill families (`data/skills/*.json` `command_groups`) AND the personas (`data/personas/*.json` `delegates_to[].command_groups`), unless the group is in the explicit cross-cutting/meta allowlist inside the test (`auth, catalog, completions, context, feedback, item, jobs, mcp, operation, profile, upgrade` — those are documented in the root skill, not a workload family). It also asserts the allowlist itself contains only real groups. This keeps skills + context knowledge from silently drifting behind a newly-added command group before a release — the manual audit, automated. To fix a failure, add the group to the right family/persona (then `cargo test generate_subskills -- --ignored`), or, only for a genuinely cross-cutting group, add it to the allowlist and the root skill.
...
- Test env vars: `FABIO_TEST_SOURCE_WORKSPACE`, `FABIO_TEST_SOURCE_LAKEHOUSE`, `FABIO_TEST_DEST_WORKSPACE`, `FABIO_TEST_DEST_LAKEHOUSE`, `FABIO_TEST_NOTEBOOK_ID`, `FABIO_TEST_CAPACITY_ID`
 ...
 - **Power BI Project (PBIP/PBIR) report authoring for coding agents**: fabio treats the documented PBIP/PBIR plain-text format (`https://learn.microsoft.com/power-bi/developer/projects/projects-overview`) as the agent-native way to author Power BI reports. Two commands close the biggest gaps: **`report validate --source <path>`** (OFFLINE structural + `$schema` validation of a `.Report` folder, a `definition.pbir`, or a PBIP root — required PBIR files, JSON validity, byPath-vs-byConnection, version/format compatibility; machine-readable `code`s; non-zero exit on invalid), and **`report create --definition <folder>`** (creates a FULL multi-page PBIR report from a generated folder — gathers all files recursively, validates first, optional `--dataset` rebinds `definition.pbir` to a concrete model by connection). Previously only `deploy` could push a full PBIR tree, and only with `.platform` scaffolding. PBIR logic lives in `src/commands/report_pbir.rs` (sibling module of `report.rs` via `#[path]`, since report.rs is near the 1500-line limit); pure helpers `validate_report_folder`/`validate`/`gather_report_parts`/`rebind_pbir_part` are unit-tested and the full export→validate→create-from-folder→render loop is live-validated. Roadmap (deferred): `.platform` synthesis so a raw Desktop PBIP (no `.platform`) is deployable, full JSON-Schema (not just structural) validation, folder-based `semantic-model` TMDL ingestion outside deploy, and PBIR scaffolding from a compact page/visual spec. See `.agents/API-BEHAVIORS-DISCOVERED.md` "Power BI Project (PBIP) / PBIR Report Support".
...
ghcr.io/iemejia/fabio:0.51.0       # release version
ghcr.io/iemejia/fabio:0.51         # major.minor
```
