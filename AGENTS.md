...
- **Git Workspace Relations (Preview)**: New `fabio git relation list|create|delete` commands implement the Fabric REST `WorkspaceRelations` API (`GET/POST /workspaces/{id}/git/workspaceRelations`, `DELETE .../{relationId}`) for managing base/branch links between workspaces as an independent resource (previously only implicit via `git branch-out`). Implemented in `src/commands/git/relation.rs` (the `git` command is a directory module — `mod.rs` enum/dispatch + `sync.rs` + `connect.rs` + `branch_out.rs` + `relation.rs`). See `.agents/API-BEHAVIORS-DISCOVERED.md` "Git Workspace Relations API Behaviors Discovered (Preview)" for API semantics and error codes.
 ...
 - **Ontology generation from a semantic model (`ontology generate`, portal-parity client-side)**: The Fabric portal's "Generate Ontology" from a semantic model has NO public REST API (the ontology REST surface is CRUD-only), so `fabio ontology generate --workspace <ws> --semantic-model <id> --name <name> [--lakehouse <id>] [--output-owl <file>]` reproduces it CLIENT-SIDE. It reads the model's tables/columns/relationships via the DAX `INFO.VIEW.*` functions (reusing `semantic-model list-tables/list-columns/list-relationships` — `fetch_info_view` is now `pub(crate)`), synthesizes an OWL model (pure `build_owl` in `src/commands/ontology/generate.rs`: table→`owl:Class`, column→typed `owl:DatatypeProperty`, relationship→`owl:ObjectProperty` many-side→one-side, relationship one-side `ToColumn`→`isIdentifier` key), and runs it through the existing `ontology import` path (create → `import_owl`), so entity types + typed properties + relationship types + keys + (with `--lakehouse`) data bindings are all produced. INFO.VIEW `DataType` maps live-verified: `Text`→string, `Integer`→long, `Number`→double (Power BI's floating-point "Decimal Number" surfaces as `Number`, NOT `Double`), Currency/Decimal→decimal, DateTime→dateTime, Boolean→boolean; the synthetic hidden `RowNumber-*` column is excluded. `import_owl` gained a `suppress_output` flag so generate emits ONE JSON object. Live-verified end-to-end: a 3-table retail model generated a 3-entity ontology with 17 typed properties, keys, 2 relationships, and queryable lakehouse `DataBindings` — reproducing the Fabric ontology tutorial's Part 1. Pure `build_owl`/`map_datatype`/`is_synthetic_column`/`summarize` are unit-tested; a live e2e (`ontology_generate_from_semantic_model`) validates create+import. The FULL tutorial (lakehouse+eventhouse+ontology+data-agent+NL query) is reproducible with fabio — see the `ontology_tutorial` workflow. See `.agents/API-BEHAVIORS-DISCOVERED.md` "Ontology generation from a semantic model".
