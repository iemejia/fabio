...
Currently registered (verified live via a tenant-settings dump): `PowerBIMCP` (semantic-model generate-dax/copilot-schema, report copilot-metadata), `ArtifactDatabricksStoragePreview` (azure-databricks-storage), `OntologyPreview` (ontology), `DigitalOperationsPreview` (digital-twin-builder), `EventDefinitionTenantSwitch` (event-schema-set), `ArtifactMirroredCatalogPreview` (mirrored-catalog), `AppBackendTenant` (app-backend), `AllowExternalDataSharingSwitch`/`AllowExternalDataSharingReceiverSwitch` (item external-data-share / accept), `PublishToWeb` (report publish-to-web), `AllowStoreAOAIDataInOtherRegions` (data-agent query/evaluate — **conditional**, see below). Live-validated on `PowerBIMCP` (subcommand-mapped), `ArtifactDatabricksStoragePreview` (group-mapped), and `EventDefinitionTenantSwitch` (group-mapped — disabling it makes `event-schema-set create` return a feature-disabled 403). **Verified portal-only (NOT registered):** `ArtifactDBTItemTenantSwitch` (data-build-tool-job) and `RTHAnomalyDetectionTenantSwitch` (anomaly-detector) gate only the portal — `create` still succeeds over REST with them disabled, so they are deliberately absent. NEVER add an unverified mapping — an unverified row is worse than none (it names the wrong setting); the generic Layer-1 hint already covers the un-registered case actionably.

**Conditional gates (`TenantSetting.conditional = true`)** — some errors are only a *likely* symptom of a disabled setting, not a definitive `403 FeatureNotAvailable`. Example: a `data-agent query`/`evaluate` whose orchestrator returns "The Data Agent run failed before producing a result." is a probable symptom of `AllowStoreAOAIDataInOtherRegions` being off (the conversational agent must store history via Azure OpenAI, gated for capacities whose region is outside the EU data boundary and the US) — but it can ALSO be a transient/paused-capacity failure. For a conditional gate the marker is narrow (only the "failed before producing a result" phrasing — the orchestrator never ran; a failure WITH output is a different problem), `enrich` keeps the original `ApiError` code (not `Forbidden`), and the message/hint use "likely cause"/"may be required" language plus the transient-failure alternative. Use `conditional: true` whenever the triggering signal is heuristic rather than an authoritative feature-disabled marker.
...
ghcr.io/iemejia/fabio:0.75.0       # release version
ghcr.io/iemejia/fabio:0.75         # major.minor
```
