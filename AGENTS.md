...
- **Connection `--creation-method` (Fabric July 2026: Event Hubs Workspace Identity, etc.)**: `connection create` gained an optional `--creation-method` flag for `connectionDetails.creationMethod`, which OFTEN differs from the connection `--connection-type` (verified live: `EventHub` → `EventHub.Contents`, `ConfluentCloud` → `ConfluentCloud.Contents`, `MQTT` → `MQTT.Contents`; `Oracle` → `Oracle` matches). fabio previously HARDCODED `creationMethod = connection_type`, which failed with `InvalidConnectionDetails` for those types and silently blocked creating Azure Event Hubs / Kafka / MQTT connections — including the July 2026 "Azure Event Hubs source with Workspace Identity authentication" Eventstream feature (the source references such a connection; auth lives in the connection, and `WorkspaceIdentity` is already a supported `--credential-type`). The flag defaults to the connection type (backward compatible); the exact value comes from `connection list-supported-types -o json` (`creationMethods[].name`). Pure `build_connection_body` in `src/commands/connection.rs` is unit-tested; live-validated (`connection_creation_method_accepted_by_api` proves the API rejects the wrong method and accepts `EventHub.Contents`). The stale `connection.json` schema doc (which advertised a nonexistent `--content` flag) was corrected to the real typed flags.
