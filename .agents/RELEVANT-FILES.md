...
- `src/commands/workspace/`: Workspace lifecycle, recoverable-item list/recover/permanent-delete, capacity, identity, role assignments, settings, networking, storage format, folders, OneLake, lifecycle policies, encryption, clone, and URL commands. `settings.rs` includes OneLake access-time tracking; `recoverable_items.rs` implements the soft-delete retention lifecycle. `context/data/examples/workspace_recover_item.json` and `workspace_onelake_settings.json` document non-trivial responses.
