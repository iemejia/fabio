...
- `src/mcp_client.rs`: Generic Model Context Protocol (MCP) CLIENT over streamable-HTTP (`McpClient::connect`/`list_tools`/`call_tool`); consumes external MCP servers; SSE + application/json response parsing; unit tests. Counterpart of `mcp serve` (fabio as MCP server)
...
- `src/commands/semantic_model/generate.rs`: generate a Direct Lake semantic model from a lakehouse/warehouse (read SQL analytics endpoint INFORMATION_SCHEMA, map types dropping unmappable, synthesize portal-EXACT Direct Lake TMDL, create + frame); pure `map_sql_type_to_powerbi`/`plan_tables`/`build_tmdl_parts`/`tmdl_model`/`tmdl_database`/`tmdl_expression`/`tmdl_table`/`build_schema_query`/`summarize` + unit tests
 ...
 - `src/commands/semantic_model/authoring.rs`: `set-description`/`add-measure`/`update-measure` (granular object editing via definition read-modify-write, no XMLA/TOM); pure `tmdl_set_description`/`add_measure_tmdl`/`update_measure_tmdl`/`render_measure_expr`/`is_child_object_decl` + `model.bim` variants + unit tests
