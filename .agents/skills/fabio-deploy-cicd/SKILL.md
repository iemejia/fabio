...
- Git status supports --include-files-details to return each item's fileChanges array. Commit specific files with --items-with-file-selection JSON; paths are relative to the item root, use forward slashes, and an empty selectedFiles array means all files in that item.
