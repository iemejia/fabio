...
- Git status --include-file-details returns fileChanges per item. Use repeatable 'git commit --file-selection ITEM_ID=PATH[,PATH...]' for FileLevelSelective commits; ITEM_ID= selects every file in that item. Paths are item-root-relative, forward-slash-only full file paths.
