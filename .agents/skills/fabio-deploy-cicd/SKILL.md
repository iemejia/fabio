...
- Per-item definition options are available across CI/CD APIs: 'git pull --item-options' and 'item bulk-import-definitions --item-options' take [{logicalId,options}], while 'deployment-pipeline deploy --item-options' takes [{sourceItemId,options}]. Each target may appear only once. Use item-type-supported option names such as validateOnly; fabio validates the array shape and target UUIDs but intentionally preserves arbitrary option objects for forward compatibility.
...
- PREFER fabio's API-driven deploy over Fabric Git integration on the PROD deploy path. fabio's 'deploy apply' (Create/Update Item Definition, content-hash diff) does NOT connect the target workspace to Git, so it avoids the two problems teams hit when a Test/Prod workspace syncs from a branch via Fabric Git integration: (1) 'ghost commits' — semantically-insignificant changes the AS engine / line-break normalization / service-side serialization re-introduces on commit, and (2) the Source Control panel showing uncommitted-change drift no user made. Dev stays Git-connected (that's where humans branch out and commit); Test/Prod receive script/API deploys only. This is the fabio-recommended hybrid — see 'context best-practices cicd-lifecycle'.
- Hotfix and rollback are Git operations + a fabio re-deploy, never a hand-edit of prod: a hotfix branches off the prod branch, deploys forward ('deploy apply --verify'), and is cherry-picked back to lower branches; a rollback reverts the bad commit and re-deploys the known-good source. A definition rollback does NOT restore DATA (git tracks definitions, not rows) — plan data recovery separately (warehouse-snapshot restore / Lakehouse Delta time-travel / ETL re-run). See 'context best-practices hotfix-rollback' and 'context workflow deploy-rollback'.
...
| `fabio context best-practices hotfix-rollback` | How to ship an urgent production hotfix and how to roll a Fabric deployment back safely with fabio. Git is the source of truth, so both flows are Git operations plus a fabio re-deploy: a hotfix branch off main deploys forward and is cherry-picked back to lower branches; a rollback reverts the commit and re-deploys the known-good source. Covers the data dimension (definitions roll back, DATA does not) and a production validation checklist. Uses deploy plan/apply, --git-diff, --verify, deployment-pipeline deploy (for non-Git-tracked items), and warehouse-snapshot (pre-deploy data backup). |
...
- fabio context workflow deploy-rollback
