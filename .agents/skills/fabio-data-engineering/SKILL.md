...
- Environment live-pool hydration is configured on staging Spark compute: set customLivePoolSupport to Enabled or Disabled, then maxClustersToHydrate (minimum 1 and no higher than instancePool.maxClustersToHydrateLimit), clusterIdleTimeout (PT20M-PT24H), and customLivePoolLifespan (PT30M-PT24H). Disabled retains settings; --clear-custom-live-pool-settings removes them.
