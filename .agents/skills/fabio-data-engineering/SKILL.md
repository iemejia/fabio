...
- Environment live pools remove Spark cold-start latency by pre-hydrating clusters. Configure staging compute with --live-pool-support Enabled plus hydration settings, then publish. maxClustersToHydrate is at least 1 and cannot exceed instancePool.maxClustersToHydrateLimit, which depends on capacity size; idle timeout is PT20M-PT24H and activation lifespan is PT30M-PT24H. Disabled retains settings; --clear-live-pool-settings removes them.
