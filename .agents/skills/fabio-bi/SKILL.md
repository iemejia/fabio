...
- semantic-model create/update-definition --allow-purge-data sends options.allowPurgeData=true. It permits definition changes that permanently purge model data which cannot be retained; refresh the model afterward to reload data.
...
- --allow-purge-data is an explicit irreversible consent: preview with --dry-run, confirm the purge, and plan a refresh before applying.
