...
- DEPLOY LIFECYCLE — data bindings travel WITH the definition. An ontology's DataBindings are parts of its item definition, so 'deploy export' captures them and 'deploy apply' (or 'ontology update-definition') reproduces them in the target workspace. A fabio-deployed ontology is therefore already bound to its data source — you do NOT need the portal's manual Graph-Model 'Get data' step that a definition-only/portal deploy requires. (Live-confirmed: a deployed ontology's get-definition returns its EntityTypes/*/DataBindings/*.json parts.)
- DEPLOY LIFECYCLE — first-time overview/graph load is PORTAL-GATED (not a fabio bug). On the FIRST deploy to a clean workspace, the ontology overview may stay on 'Setting up your ontology — We are preparing the ontology overview for the first time', and the backing GraphModel's data load isn't provisioned until the item is opened once in the Fabric portal (the same 'VersionConfig' gate 'graph-model initialize' reports for graph models, and the data-agent publish gate). This is a Fabric platform limitation on initial deployment, NOT a deploy failure — the definition (types + bindings) deployed correctly. Workaround: open the ontology/graph once in the portal to initialize it (the portal 'rename an entity type and rename it back' trick just forces that same overview initialization); subsequent deploys need no manual step.
...
| After deploying an ontology to a clean workspace, its overview is stuck on 'Setting up your ontology' / the graph isn't queryable yet | This is a PORTAL-GATED first-time initialization, not a deploy failure — the definition (entity/relationship types + DataBindings) deployed correctly (confirm with 'ontology get-definition'). The overview/graph load is only provisioned when the item is opened once in the Fabric portal (same 'VersionConfig' gate as 'graph-model initialize' and the data-agent publish gate). Open the ontology/graph once in the portal to initialize it (the portal 'rename an entity type, then rename it back' trick just forces that overview init). Do NOT re-run deploy expecting it to fix this — the data bindings are already present; only the portal-side load is pending. Subsequent deploys need no manual step. |
