...
- SQL Audit predicate filtering is available on ALL THREE T-SQL surfaces: 'warehouse', 'sql-endpoint', and 'sql-database' update-audit-settings all accept '--predicate-expression' (a T-SQL WHERE-style filter, e.g. "database_principal_name <> 'dbo'", stored as top-level predicateExpression) to keep only the audit rows that matter. Pair it with '--state Enabled', '--retention-days', and '--audit-actions'. Writing audit settings needs Contributor+ AND an elevated audit scope — a correctly-formed request may still return FORBIDDEN/InsufficientScopes on a low-privilege token.
