...
- 'sql-endpoint refresh-metadata --tables' accepts a JSON array of {schema,tableNames} selectors (inline or @file), with at most 25 total tables. Its optional --timeout is a Duration JSON object with numeric value and a PascalCase timeUnit (Seconds, Minutes, Hours, or Days), defaults to 15 minutes, and cannot exceed 24 hours. Schema-enabled endpoints return schema-qualified tableName values; non-schema-enabled endpoints resolve under the default schema.
...
- SQL Audit predicate filtering is available on ALL THREE T-SQL surfaces: 'warehouse', 'sql-endpoint', and 'sql-database' update-audit-settings all accept '--predicate-expression' (pass only the expression, WITHOUT WHERE; maximum 3,000 characters; e.g. "database_principal_name <> 'dbo'", stored as top-level predicateExpression) to keep only the audit rows that matter. An empty string removes the predicate; omitting the flag leaves an existing predicate unchanged. Pair it with '--state Enabled', '--retention-days', and '--audit-actions'. Writing audit settings needs Contributor+ AND an elevated audit scope — a correctly-formed request may still return FORBIDDEN/InsufficientScopes on a low-privilege token.
