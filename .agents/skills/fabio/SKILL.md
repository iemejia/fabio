...
```bash
# Find commands — compact index of all 80 groups + subcommands
fabio context agent
...
- **`--item-options` / `--allow-purge-data`**: `--item-options` (on `deployment-pipeline deploy`, `git pull`, `item bulk-import-definitions`, `workspace clone`) is a JSON array of `{<idField>, options}` entries — `sourceItemId` for deploy, `logicalId` elsewhere — each a **canonical 36-char UUID**, inline or `@file`. A semantic-model entry's `options.allowPurgeData: true` (or the typed `--allow-purge-data` flag on `semantic-model create`/`update-definition`, or the generic `item create --definition --options '{"allowPurgeData":true}'` / `item update-definition --options '{"allowPurgeData":true}'`) permits Fabric to **permanently purge** incompatible data that cannot be retained. Any purge-enabling invocation — typed flag, generic `--options`, or nested per-item option — marks the `--dry-run` preview `"destructive": true` with a `warning` + `agentNotice` and repeats the `warning` on the real success output — treat it as a hard stop and confirm with the user first. After a purge-enabling `semantic-model`/`item` `create`/`update-definition`, **refresh the model** (`fabio semantic-model refresh --workspace $WS --id $ID`) so it reframes over the retained data.
