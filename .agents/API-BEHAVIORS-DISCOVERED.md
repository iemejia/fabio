...
- **Power BI MCP feature-disabled signature (`FeatureNotAvailable` at initialize)**: When the `PowerBIMCP` tenant setting is OFF, the MCP `initialize` handshake returns **HTTP 403** with the JSON-RPC error `{"code":-32003,"message":"The feature is not available","data":{"errorCode":"FeatureNotAvailable"}}` — i.e. the failure is at CONNECT, before any tool call. (A model/report *permission* problem, by contrast, surfaces later at `call_tool`.) So a connect-time 403 is a reliable "feature disabled" signal, and fabio maps it (`powerbi_mcp::is_feature_disabled`) to a teaching `FORBIDDEN` error naming the exact setting + the non-MCP fallbacks, with `hintType: semantic_correction` so an agent stops retrying. Live-verified: after `admin update-tenant-setting --setting-name PowerBIMCP --content '{"enabled":false}'`, `generate-dax`/`copilot-schema`/`report copilot-metadata` all return the teaching error; re-enabling restores the happy path.
