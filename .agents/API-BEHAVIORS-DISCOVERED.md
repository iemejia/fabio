...
- **New commands added**: `fabio eventstream add-source` and `fabio eventstream add-destination` — high-level helpers that fetch current definition, merge in the new node, auto-create default streams, and push the updated definition. Simplifies agent workflow vs. manually crafting full definition JSON.
 ...
 - **Eventstream `add-operator` gotchas (input-node = the source's DEFAULT STREAM, not the source; DataType enum) — confirmed live**: Two non-obvious requirements an agent hits when adding an operator, both server-validated (fabio passes `--properties` through faithfully and surfaces the exact server error — dry-run shows the built body is correct). (1) **`--input-node` must reference a STREAM node, not a source.** A source (e.g. `add-sample-source --name bikes`) auto-creates a default stream named `<source>-stream` (`get-topology` → `streams:[{name:"bikes-stream", type:"DefaultStream"}]`). Passing `--input-node bikes` fails with `WorkloadUserException: The operator '…' contains invalid input node(s): bikes. Input node names must be valid stream…`; use `--input-node bikes-stream` (and the Filter condition's `column.node` must also be `bikes-stream`). (2) **The Filter condition `value.dataType` enum is `BigInt | Float | Nvarchar(max) | DateTime | Bit | Record | Array | Any`** (from `operatorCommon.json#/definitions/DataType`) — NOT `Number`/`Int`/`String` (`Error converting value "Number" to type '…Operators.DataType'`). For a numeric column use `BigInt` (integers) or `Float`. Live-validated: `add-operator --type Filter --input-node bikes-stream --properties '{"conditions":[{"column":{"node":"bikes-stream","columnName":"No_Bikes","columnPath":[]},"operatorType":"GreaterThan","value":{"dataType":"BigInt","value":"5"}}]}'` → the Filter lands, `get-topology` → `operators:[("filterBikes","Filter", inputNodes:[{name:"bikes-stream"}])]`.
  - **Eventhouse destination: topology authoring works via REST, but the Kusto data-connection provisioning is portal-gated (live-observed Aug 2026)**: `add-destination --destination-type Eventhouse --input-node <stream> --properties '{dataIngestionMode:"DirectIngestion", workspaceId, itemId:<kqlDbId>, tableName, inputSerialization:{type:"Json"}}'` successfully writes the destination into the topology (source→stream→Eventhouse graph is correct; `get-topology` reflects it) and the SOURCE transitions to `Running`. HOWEVER the Eventhouse destination stays in `status:"Warning"` with `connectionName:null, mappingRuleName:null`, ingests **0 rows** (verified over ~7 min, even after pre-creating the destination table with the exact sample schema in the KQL DB), logs no `.show ingestion failures`, and `get-destination-connection` returns `NOT_FOUND: Connection is not available for the type 'Kusto'`. Conclusion: the underlying **Kusto data connection** that the Fabric portal auto-provisions when you add an Eventhouse destination is NOT triggered by the REST `updateDefinition`/topology path alone — so a purely REST-authored eventstream→eventhouse pipeline does not ingest. This mirrors the other portal-assisted provisioning gates (graph-model load, ontology Fabric-IQ reasoning). The topology/authoring surface itself is correct and fully reproducible; only the live data-connection wiring is portal-gated. Open question for a future epic: is there a separate Fabric REST connection-provisioning/publish call fabio could make to complete the wiring?
