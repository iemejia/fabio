...
- **Querying a Databricks MANAGED table via the mirror SQL endpoint fails on deletion vectors (Databricks-Fabric interop limitation, NOT a fabio bug)**: The SQL-endpoint metadata sync for a mirrored table (`sql-endpoint refresh-metadata` → per-table `Failure`) returns a storage `BadRequest` (`Status: 400 … x-ms-remote-data-destination-type`) for a Unity Catalog table created with the modern defaults `delta.enableDeletionVectors=true` + `delta.minReaderVersion=3`. Fabric's SQL endpoint reads the mirrored table through a OneLake shortcut to the Databricks storage and cannot read the deletion-vector reader-v3 protocol. The fix is Databricks-side: recreate the table with `TBLPROPERTIES ('delta.enableDeletionVectors' = false)` (drops it to `minReaderVersion=1`), or use EXTERNAL tables in a Fabric-accessible ADLS location, or supply a `--storage-connection-id`. NOTE the mirror re-replicates a recreated table on the autoSync interval (not instantly), so `lastSyncDateTime` must advance past the recreation before the DV-free table is queryable. The fabio surface (activation + endpoint resolution) is complete; the remaining constraint is the platform's.
