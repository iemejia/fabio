...
- **Item logical IDs are now first-class metadata and mutable for conflict resolution**: Item create/get/update/list responses include nullable `logicalId`; `null` means no logical ID has been assigned. `PATCH /workspaces/{workspaceId}/items/{itemId}/logicalId` requires `{"logicalId":"<uuid>"}` and returns `{"logicalId":"<uuid>"}`. The GUID must be non-zero and unique within the workspace. The operation requires item read+write permission, supports users/service principals/managed identities, and fails with `ActiveCiCdOperationInProgress` while a workspace CI/CD operation is active. Changing it can break automations that use the old logical ID or create duplicate-name lookup failures, so review dependent automations first. Fabio exposes this as `item update-logical-id` with local UUID/empty-GUID validation and `--dry-run`.
...
- **Bulk operations (all LRO)**:
  - `bulkExportDefinitions` and `bulkImportDefinitions` accept request payloads up to 128 MB. Requests containing any Fabric item type require a workspace on Fabric capacity (F SKU, including Fabric Trial); Power BI Premium capacity is sufficient only when every item is a Power BI item. Fabio rejects oversized serialized requests locally.
