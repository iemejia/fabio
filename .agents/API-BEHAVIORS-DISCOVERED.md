...
- **Restore points — two request-body bugs found & fixed (verified live against the spec + tenant)**: (1) `create-restore-point`'s `CreateRestorePointRequest` body is `{displayName, description}` — fabio was sending `{restorePointLabel}`, a field NOT in the schema, so `--name` was silently IGNORED and every restore point was named the default "Restore point" (live-confirmed). Fixed to send `displayName` (+ a new `--description`). The create endpoint returns `201` with the `RestorePoint` OR `202` (async) with an EMPTY body; fabio now uses `poll:true` and, if the result lacks an `id`, re-fetches the list and returns the newest restore point (max id — ids are creation-time epochs) so an agent always gets an id back (previously it returned `{"data":null}`). (2) `restore-to-point` (`POST /restorePoints/{id}/restore`) takes **NO request body** — only the restore-point id in the path (restore is IN-PLACE). fabio was REQUIRING a bogus `--name` and sending `{restoreToWarehouseName}` the server ignores; removed `--name`, now sends an empty body and reports `{status:"restored"}` on the empty async response. NOTE the restore operation itself needs an elevated tenant scope (a device-code token can hit `FORBIDDEN: InsufficientScopes`), but the create/label/response fixes are fully live-verified.
