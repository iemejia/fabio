...
## Fabric REST spec 8baaa4c — definition options, audit predicates, and structured errors

- **No new endpoints were introduced; existing request contracts were extended.** Deployment-pipeline `deploy` now accepts `options.itemOptionsBySourceItemId[]` entries keyed by `sourceItemId`; Git `updateFromGit` and `bulkImportDefinitions` accept `options.itemOptionsByLogicalId[]` keyed by `logicalId`. Every entry requires an `options` object, and a target item may occur at most once. The option object is intentionally open-ended because valid names depend on the item type; the new upstream examples use `validateOnly:true`.
- **Generic item definition options are conditional on a definition.** Item creation accepts top-level `options` only when `definition` is also present; options without a definition produce `BadRequest`. `updateDefinition` accepts the same top-level open-ended `options` object. Semantic models define `allowPurgeData` as their typed option: when true, definition changes may purge existing data that cannot be retained, and the model must be refreshed afterward. The default is false.
- **SQL audit predicates are a top-level audit setting.** GET responses can contain `predicateExpression`; PATCH requests may set it. Supply only the predicate expression, never the `WHERE` keyword. Maximum length is 3,000 characters. On first enablement omission means no predicate; on later updates omission preserves the existing predicate; an empty string removes it. `action_id` and `class_type` are returned as strings but predicates may compare them only to numeric values, and string comparisons do not perform implicit conversion. All documented audit-record fields are allowed except `file_name`, `audit_file_offset`, and `event_time`.
- **SQL endpoint refresh timeout is bounded.** The duration still defaults to 15 minutes and accepts `Seconds`, `Minutes`, `Hours`, or `Days`, but the documented maximum is now 24 hours.
- **Fabric errors can include `error.parameters[]`.** Each entry can carry `name`, `value`, and `message`; all three are read-only and individually optional. fabio preserves these entries in the structured stderr envelope, including LRO failures. The upstream SQL-audit example also corrected the update response fixture to place the settings object under `responses.200.body`; the runtime response schema itself did not change.
