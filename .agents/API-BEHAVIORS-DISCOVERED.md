...
- **`semantic-model generate` (Direct Lake from a data source, portal-parity, no REST API) — live-confirmed**: The Fabric portal's "New semantic model" on a lakehouse/warehouse/SQL analytics endpoint (open item → pick tables → Direct Lake model) has NO public REST API. fabio reproduces it CLIENT-SIDE: resolve the source's SQL analytics endpoint `(server, database)` → read `INFORMATION_SCHEMA.COLUMNS` for BASE TABLEs over TDS (optionally filtered to `--tables`, `--schema` default `dbo`) → map each SQL type to a Power BI column `dataType`, **dropping unmappable columns** (varbinary/geography/geometry/hierarchyid/sql_variant/xml/… → dropped, matching Fabric's own sync rule) → synthesize a Direct Lake `model.bim` → create → frame with a `Full` refresh. Live-validated end-to-end against a populated lakehouse: generate → `INFO.VIEW` lists the tables → a real DAX query (`SUMMARIZECOLUMNS('dimstore'[StoreName], …)`) returns actual OneLake data via zero-copy Direct Lake. Relationships/measures are NOT inferred (same as the portal — add them via `update-definition`). Reads the schema over TDS, so it needs a **SQL-scoped token from the ambient credential chain** (az/device-code cache), NOT a Fabric-only static `FABIO_ACCESS_TOKEN`. Mirrors `ontology generate` (which likewise reproduces a portal-only feature). Pure helpers (`map_sql_type_to_powerbi`, `plan_tables`, `build_direct_lake_bim`, `build_schema_query`, `summarize`) in `src/commands/semantic_model/generate.rs` are unit-tested; the full generate→frame→DAX loop is `semantic_model_generate_direct_lake_lifecycle` (gated on `FABIO_TEST_LOADED_LAKEHOUSE`).
- **model.bim CAN express Direct Lake at compat 1604 (corrects an earlier note) — live-confirmed**: A `model.bim` (TMSL JSON) with `compatibilityLevel: 1604`, `model.defaultPowerBIDataSourceVersion: "powerBI_V3"`, `model.defaultMode: "directLake"`, one `directLake` **entity** partition per table (`source: {type: "entity", entityName, schemaName, expressionSource: "DatabaseQuery"}`), and a shared `DatabaseQuery` M expression (`Sql.Database("<sqlEndpointHost>", "<catalog>")`) IS accepted by the Fabric Items API and produces a queryable Direct Lake model (proven by `semantic-model generate`, which emits exactly this shape and returns live DAX results). So Direct Lake does NOT strictly require TMDL — the earlier "Direct Lake REQUIRES TMDL" was true only for the OLD `model.bim` at compat **1550** (which lacks V3/`directLake` support); at **1604/powerBI_V3** the JSON form works. TMDL remains preferable for hand-authoring multi-table models (one file per table, richer surface), but for programmatic generation the single-blob model.bim is simpler and functional.
