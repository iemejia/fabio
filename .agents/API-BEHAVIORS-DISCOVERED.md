...
- **Connection string format**: Returns the DW-style endpoint hostname (e.g., `*.datawarehouse.fabric.microsoft.com`).
 - **refresh-metadata returns table sync status**: The `POST /workspaces/{ws}/sqlEndpoints/{id}/refreshMetadata` LRO returns `{"value":[{tableName, status, startDateTime, endDateTime, lastSuccessfulSyncDateTime}]}` — a per-table sync-result list. `status` is `Success` (synced this run), `NotRun` (already up to date), or `Failed`. `sql-endpoint refresh-metadata` renders the `value` array as a proper list (`{data:[…], count}` + table view) so agents can filter (e.g. `--query "[?status!='Success']"` to find tables that failed/didn't sync). Live-verified: a freshly loaded table shows `Success` on the first refresh and `NotRun` on an immediate second refresh.
