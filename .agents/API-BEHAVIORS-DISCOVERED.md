...
- **Tenant-setting override APIs are GA (September 2026)**: tenant-setting update plus capacity/domain/workspace delegated-override list, update, and delete endpoints no longer carry Preview warnings. Existing permissions, pagination limits, and 25/100-requests-per-minute constraints are unchanged.

## Workspace Recoverable Items API Behaviors
- **Soft-deleted item lifecycle (Aug 2026 spec)**: `GET /workspaces/{workspaceId}/recoverableItems` returns a paginated `value` list with optional `type` and `recoverableByMe` query filters. Each recoverable item requires `id`, `type`, and `retentionExpirationDateTime`; it may also include `displayName`, `description`, `workspaceId`, `parentItemId`, and `tags`. `recoverableByMe=true` limits results to items the caller can recover immediately.
- **Recovery is an LRO with partial success**: `POST /workspaces/{workspaceId}/recoverableItems/{itemId}/recover` takes an empty JSON body, returns an `Items` collection on synchronous success, and may return 202 with standard `Location`, `x-ms-operation-id`, and `Retry-After` headers. Recovery includes child items, but if one child fails, that child and its descendants remain soft-deleted while independent branches may still recover. Service-principal and managed-identity support depends on the item type.
- **Recoverable-item deletion is permanent**: `DELETE /workspaces/{workspaceId}/recoverableItems/{itemId}` permanently removes the soft-deleted item and returns an empty 200 response. The API can report `InvalidTargetItemStateForSoftDeletion`; recovery additionally reports `InvalidParentItemStateForRecovery` and `InvalidTargetItemStateForRecovery`.
- **Gated by the `ConfigureArtifactRetentionPeriod` tenant setting ("Fabric item recovery") — a FUNCTIONAL gate, not an API gate (live-verified Aug 2026)**: The recycle bin only exists when an admin enables `ConfigureArtifactRetentionPeriod` and sets its `ArtifactRetentionPeriod` (days). When it is DISABLED, `item delete` (soft delete) is effectively permanent — the item NEVER enters `recoverableItems`. Crucially, `list-recoverable-items` does NOT return a `403 FeatureNotAvailable` in that state — it returns an empty list (`{"data":[],"count":0}`). Because there is no feature-disabled 403, this does NOT belong in `src/commands/tenant_gate.rs` (the generic gate keys off a 403); an agent must instead recognize "delete succeeded but the item never appears in `list-recoverable-items`" as the disabled-recycle-bin signal and, if recovery is needed, ask an admin to enable `ConfigureArtifactRetentionPeriod`. Enabling body: `{"enabled": true, "properties": [{"name": "ArtifactRetentionPeriod", "value": "7", "type": "Integer"}]}` (value = retention days). Live-proven: with the setting OFF a soft-deleted Lakehouse never appeared after 60s of polling; immediately after enabling (propagation was near-instant in this tenant, though Fabric documents up to ~15 min), the same soft-delete appeared with `retentionExpirationDateTime` set, recovered successfully (recover is an LRO taking ~20-30s), and `delete-recoverable-item` purged it permanently. The live lifecycle e2e tests (`workspace_recover_item_live_lifecycle`, `workspace_delete_recoverable_item_live_lifecycle`) therefore REQUIRE this setting enabled on the target tenant.

## Ontology API Behaviors Discovered
- **`ontology import` silently dropped self-closing OWL elements (bug fixed, live-verified Aug 2026)**: fabio's RDF/XML parser (`parse_rdf_xml`, quick_xml) only PUSHED a class/property to the model on `Event::End`, but a self-closing element — `<owl:Class rdf:about="X"/>` (a class with no inline `rdfs:label`/`subClassOf`, which is perfectly valid and COMMON RDF/XML) — fires `Event::Empty`, NOT `Start`+`End`. So such classes/properties were parsed as a start (fields set) but never finalized → the import reported `status: imported, entity_types: 0` with NO error, silently dropping the whole ontology. Live-repro: a hand-written OWL with `<owl:Class .../>` self-closing classes imported 0; the SAME OWL with non-self-closing `<owl:Class>…</owl:Class>` imported 2 entities + 1 relationship. (`ontology generate`'s `build_owl` always emits a child `<rdfs:label>`, so its classes are never self-closing — which is why generate worked and hid the bug.) Fixed by finalizing self-closing elements in the `Event::Empty` branch (shared `finalize_owl_class`/`_datatype_property`/`_object_property` helpers, reused by the `End` arm). A label-less class correctly falls back to the IRI local name. ALSO: `import` now emits a teaching `hint` when it extracts 0 entity types (was a silent success) — pointing out that **Turtle/.ttl is NOT supported by `ontology import`** (only RDF/XML `.rdf`/`.owl`/`.xml` and JSON-LD `.jsonld`/`.json`; convert first). NOTE a format-support INCONSISTENCY remains: `ontology update-definition`'s error hint claims `.ttl`/`.nt`/`.n3`/`.trig` are supported while `ontology import` rejects them — the two OWL-ingestion paths advertise different format sets (candidate for future alignment). Unit-tested (`test_parse_rdf_xml_self_closing_elements`); live-validated end-to-end.
- **`valueType` enum is EXACTLY `String`/`Boolean`/`DateTime`/`Object`/`BigInt`/`Double` — `Long`/`Decimal`/`Any` are INVALID for a typed property (bug fixed, live-verified Aug 2026)**: The published `entityType/1.0.0` schema's `EntityTypeProperty.valueType` enum is `['String','Boolean','DateTime','Object','BigInt','Double']`. An entity type whose `properties[].valueType` is `Long`, `Decimal`, or `Any` is REJECTED by `updateDefinition` with a generic `ALMOperationImportFailed: Import of the {0} artifact '{1}' threw an exception with this message: {2}` (Fabric's error template ships with UNFILLED `{0}{1}{2}` placeholders — an opaque Fabric-side bug that hides the real cause). Live-proven per-type: `String`/`BigInt`/`Double`/`Boolean`/`DateTime` are accepted (as non-key properties); `Long`/`Decimal`/`Any` are rejected. `Any` is valid ONLY inside `untypedProperties` (a separate sub-schema), never in the typed `properties[]` array. **This broke `ontology add-entity-type` for EVERY integer/decimal property**: fabio's `normalize_value_type` mapped `long`/`int`/`integer`→`"Long"`, `decimal`/`currency`→`"Decimal"`, `any`→`"Any"` (all invalid) and listed `Long`/`Decimal`/`Any` in `VALUE_TYPES` while OMITTING valid `Object`. Fixed: `long`/`int`/`integer`/`bigint`→`BigInt`, `double`/`float`/`number`/`decimal`/`currency`→`Double`, `object`/`any`→`Object`, so every reasonable input maps to a schema-valid type. The `import`/`generate` paths were already correct (`playground_type_to_fabric` maps `long`→`BigInt`, `decimal`→`Double`), which is why `ontology generate` worked and hid the bug; the `add-entity-type` e2e used only `String`/`Double` so it never exercised the broken aliases. NOTE also: a KEY property (`entityIdParts`) must be `String` or `BigInt` — a `Double`/`Boolean`/`DateTime` key is rejected with `ALMOperationImportFailed: Entity keys[0] reference...` (distinct error). And an entity-type `id`/property `id` may be a ~19-digit nanosecond `InternalIdentifier` (fabio's `gen_unique_id`) — the ID length/format is NOT the issue (live-proven both 13- and 19-digit ids are accepted).
...
- **DFS directory scoping — `list-files --path` was broken, now fixed (verified live)**: The OneLake DFS filesystem-list with the ITEM-IN-PATH shape (`/{ws}/{item}?resource=filesystem&recursive=true&directory=<subdir>`) does NOT scope `directory` to a subtree — with `directory=Files/data` it returned the ENTIRE lakehouse with a bogus `Files/data/` prefix on every path (including the root `Tables`/`Functions` dirs). So `lakehouse list-files --path <subdir>` (via `client::list_onelake_files`) ignored the filter. Fixed to the WORKSPACE-SCOPED shape (filesystem = workspace, `/{ws}?resource=filesystem&recursive=true&directory={item}/<subdir>`), which correctly scopes to the subtree — the same shape `list_onelake_dir` uses. Now `list-files --path Files/data` returns only `{item}/Files/data/...`; root (no `--path`) is `directory={item}` → the full `{item}/...` tree (unchanged). Live-verified before/after.
...
- **`update-datasource --description` was silently ignored — wrong field name (bug fixed)**: fabio's `update-datasource` sent the datasource description as `userDescription`, but the runtime `PATCH /workspaces/{ws}/dataAgents/{id}/staging/datasources/{dsId}` endpoint uses `description` (matching both `UpdateDataAgentDatasourceRequest` and `DataAgentDatasourceResponse` in the spec — `userDescription` appears NOWHERE in the dataAgent API spec). Like the catalog bug, the API SILENTLY IGNORES the unknown `userDescription` field, so `--description` did nothing. Live-proven via raw `rest call` PATCH: `{userDescription:"AAA"}` → not persisted (GET shows no description); `{description:"BBB"}` → GET shows `description:"BBB"`. Also confirmed `instructions` (not `dataSourceInstructions`) is the correct field for `--instructions` (both persist per the spec). Fixed `userDescription`→`description`; live-verified `update-datasource --description "FIXED-desc"` now persists (GET → `description:"FIXED-desc"`). NOTE the agent's STORED DEFINITION format (the `Files/Config/.../datasource.json` part) uses different field names (`userDescription`/`dataSourceInstructions`) — a separate schema layer from the runtime PATCH API; the e2e definition fixtures correctly keep those.
- **Datasource schema is fetched at `add-datasource` time — add it AFTER the source's tables are queryable, or the agent has no data (tutorial-reproduction finding)**: When adding a Lakehouse (or Warehouse/SQL) datasource, the agent fetches the source's schema immediately. If the lakehouse SQL analytics endpoint hasn't yet SYNCED newly-loaded Delta tables, the datasource is created with an EMPTY schema — the staging `elements` tree returns a single degenerate `Schemas` node with `hasSubElements: false` (and, in that empty state, a placeholder `{path, name, selected}` element shape instead of the normal `{id, isSelected, displayName, hasSubElements}`), so `select-tables --all-tables` finds nothing (`modified: 0`) and the PUBLISHED agent answers "the query tool was not found or unavailable" (it can even DESCRIBE tables it read from the `instructions` text while having none actually selected — a hallucination, not grounding). Fix: wait for the SQL endpoint to sync (a direct `lakehouse query "SELECT COUNT(*)…"` succeeding confirms readiness), or `remove-datasource` + re-`add-datasource`, THEN `select-tables` (now `modified: N`) + re-`publish`. Reproduced live end-to-end with the AdventureWorks tutorial: after re-adding the synced datasource, `data-agent query "how many rows in dimcustomer?"` returned the grounded `18,484` (matching the direct SQL). This is a TIMING property of the Fabric API, not a fabio bug.
- **Fewshot example-query flag accepts the source-language aliases**: `data-agent add-fewshot`/`update-fewshot` carry the example QUERY in the `--answer` flag (a fewshot is a question→query pair; a `--query` flag would collide with the global JMESPath `--query`). To match the uniform query-language convention, the flag has visible aliases `--sql`/`--kql`/`--dax`/`--gql` — use the one matching the datasource (SQL for lakehouse/warehouse, KQL for a KQL database, GQL for graph/ontology). Semantic-model datasources do NOT support fewshots (Fabric limitation).
...
- **Definition options can explicitly consent to data purge (spec commit `8baaa4c`)**: Semantic model create-with-definition and `updateDefinition` accept `options.allowPurgeData` (default `false`). Some definition changes cannot retain existing model data; `true` allows those changes and permanently purges the incompatible data. Refresh the model after the operation to reload data. fabio exposes this as `semantic-model create/update-definition --allow-purge-data`; the flag is classified as a dangerous safety bypass, and both mutations remain dry-run guarded.
- **Direct Lake on OneLake vs on SQL — the ONLY TMDL differences (live-verified, Power BI GA March 2026)**: `semantic-model generate --storage-mode onelake` produces a **Direct Lake on OneLake** model instead of the default **Direct Lake on SQL**. The two definitions are byte-identical EXCEPT (1) `definition/expressions.tmdl` — SQL mode emits `DatabaseQuery = Sql.Database("<server>","<sqlEndpointId>")`; OneLake mode emits `DatabaseQuery = AzureStorage.DataLake("https://onelake.dfs.fabric.microsoft.com/<workspaceId>/<itemId>")` (the lakehouse/warehouse item ROOT — NOT `/Tables`, NOT the SQL-endpoint GUID) — and (2) the per-table `directLake` entity partition: OneLake mode DROPS the `schemaName:` line for a **schema-less lakehouse** while keeping `entityName:` + `expressionSource: DatabaseQuery`; SQL mode always keeps `schemaName`. This exactly matches Fabric's documented DL-on-SQL→DL-on-OneLake TMDL-view migration (`fabric/fundamentals/direct-lake-power-bi-desktop`: "Change `Sql.Database(...)` to `AzureStorage.DataLake(...)`; if the source is a Lakehouse without schemas, remove all `schemaName` references"). Schema-enablement is read from the lakehouse GET: `properties.defaultSchema` present/non-empty ⇒ schema-enabled (keep `schemaName`); absent ⇒ schema-less (drop it). A **warehouse** is always schema-enabled (keeps `schemaName: dbo`). `compatibilityLevel` stays 1604 and `model.tmdl` (`defaultMode: directLake`) is unchanged — the framing `Full` refresh and DAX-query path are identical. **Live-verified end-to-end against the tenant**: (a) schema-less lakehouse `BiLake` (table `Sales`) → definition confirmed `AzureStorage.DataLake(".../<ws>/<lakehouseId>")`, partition had `entityName: Sales` + `expressionSource: DatabaseQuery` and **NO** `schemaName`, `EVALUATE ROW("rows", COUNTROWS('Sales'), "total", SUM('Sales'[Amount]))` returned `{rows:4, total:580}`; (b) warehouse `fabio_nl2sql_wh_fixture` (table `Orders`) → partition KEPT `schemaName: dbo`, query returned 3 rows. Both models framed on create (`framed:true`) and were queryable without extra config. The `storageMode` field in `generate`'s output/dry-run is now `directLakeOnSql`/`directLakeOnOneLake` (was the flat `directLake`). Prefer OneLake (OneLake security, more modeling features, faster queries, multiple sources); use SQL only for SQL-endpoint security rules or DirectQuery fallback.
- **Remote Power BI MCP server (`{fabricBase}/mcp/powerbi`) — fabio as an MCP client for Copilot-powered tools**: The remote Power BI MCP server (streamable HTTP, Entra OAuth, `serverInfo: PowerBIMCPServer v1.0.0`) is NOT a REST API — it is consumed over MCP. A live `initialize` + `tools/list` (with the standard Fabric bearer token) returns FOUR tools: **`ExecuteQuery`** `{artifactId, daxQueries[], maxRows?}` (duplicates fabio's `semantic-model query --dax`), **`GenerateQuery`** `{artifactId, userInput, schemaSelection?, chatHistory?, valueSearchResults?}` (Copilot NL→DAX — returns `{daxQuery, semanticModel}`), **`GetSemanticModelSchema`** `{artifactId}` (serialized schema + author custom instructions + AI-optimized metadata), **`GetReportMetadata`** `{reportObjectId}` (synthesized report schema: `{Workspace, SemanticModel, ReportMetadata:{Pages:[{Title,Visuals:[{Projections,Sort}]}]}, PromptInstructions}`). `artifactId` is the model OR report GUID (global; no workspace param). **Response shape gotcha**: each tool returns MULTIPLE MCP text content blocks (e.g. `GetSemanticModelSchema` → a `{schema,semanticModel}` block AND a separate `{artifact_citation}` block); concatenating them and parsing as one JSON fails with serde "Extra data" — parse each block and merge the objects (fabio's `powerbi_mcp::tool_text_as_json`). **Prerequisite**: the tenant setting `PowerBIMCP` ("Users can use the Power BI Model Context Protocol server endpoint (preview)") must be enabled; `GenerateQuery` additionally needs a Copilot license (the org's Copilot capacity). Live-validated end-to-end: `GenerateQuery("total revenue by region")` on `RetailSalesModel` produced a clean `DEFINE MEASURE … EVALUATE SUMMARIZECOLUMNS('dimstore'[Region], "Total Revenue", [Total Revenue]) ORDER BY …`; executing a generated query against a STALE Direct Lake model surfaces `DatasetExecuteQueriesError … relationship … does not hold any data because it needs to be recalculated` (a model-refresh state issue, correctly surfaced by fabio's `--execute`), while a framed model returned real rows (France 309.83 / Germany 879.47 / USA 1089.44). fabio wraps the 3 non-redundant tools as `semantic-model generate-dax`/`copilot-schema` and `report copilot-metadata`.
- **Power BI MCP feature-disabled signature (`FeatureNotAvailable` at initialize)**: When the `PowerBIMCP` tenant setting is OFF, the MCP `initialize` handshake returns **HTTP 403** with the JSON-RPC error `{"code":-32003,"message":"The feature is not available","data":{"errorCode":"FeatureNotAvailable"}}` — i.e. the failure is at CONNECT, before any tool call. (A model/report *permission* problem, by contrast, surfaces later at `call_tool`.) This is one instance of a GENERAL pattern: a disabled tenant setting yields an opaque `403 FeatureNotAvailable`. fabio handles it GENERICALLY via `commands::tenant_gate::enrich` (wired into `commands::execute`) — a marker-based `is_feature_disabled` check turns it into a teaching `FORBIDDEN` (`hintType: semantic_correction`) that names the setting (from a command→setting registry), gives an admin-aware enable command (probing `GET /admin/tenantsettings`), and appends feature-specific fallbacks. The PowerBI-specific mapping that used to live in `powerbi_mcp.rs` was REMOVED in favor of this generic path (DRY). Live-verified across two settings: disabling `PowerBIMCP` (subcommand-mapped: `semantic-model generate-dax`) AND `ArtifactDatabricksStoragePreview` (group-mapped: `azure-databricks-storage create`) each yields the precise named-setting admin-aware error; a normal `NOT_FOUND` is NOT tenant-enriched; re-enabling restores the happy path.
- **Calculated columns are NOT supported on Direct Lake tables (platform restriction, not a fabio bug)**: `semantic-model add-calculated-column` on a table in a **Direct Lake** model is rejected server-side by `updateDefinition` with `Dataset_Import_FailedToImportDataset: … Standard expression context may not be used for calculated columns in Direct Lake tables, set for column '<name>' in table '<table>'`. This is a Fabric/AS Direct Lake limitation (a Direct Lake table's columns are backed directly by the OneLake Delta files; a standard-context calculated column would require import-mode evaluation). fabio surfaces the server message faithfully — the command itself is correct and works on Import/DirectQuery models. Discovered validating the semantic-model granular-authoring surface against a `semantic-model generate`-produced Direct Lake model, where `add-measure`, `add-relationship`, `add-role`/`set-rls`, and `add-hierarchy` all succeed and `evaluate-measure "Total Amount" --group-by dimstore.Region` returns the joined result (`EU:150, US:80`) — only calculated columns are blocked by Direct Lake.
- **`bind-connection`/`unbind-connection` need a full `connectionBinding` object, not a flat `{connectionId}` — the commands were broken (found & fixed live)**: `POST .../semanticModels/{id}/bindConnection` takes a `BindSemanticModelConnectionRequest` = `{"connectionBinding": {...}}`, where `ConnectionBinding` is `{id?, connectivityType, connectionDetails:{type, path}}` (`connectionDetails` REQUIRED — it's the `{type, path}` that matches the model's data-source reference). fabio sent a flat `{"connectionId":<id>}` for bind and `{"connectionId":null}` for unbind — both rejected with `InvalidInput: Semantic model bind request cannot be null in the request body`. Fixed: `bind-connection` now GETs `/connections/{id}` (whose response already carries `connectivityType` + `connectionDetails:{type,path}`) and builds `{connectionBinding:{id, connectivityType, connectionDetails}}`; `unbind-connection` gained a required `--connection-id` and sends `{connectionBinding:{connectivityType:"None", connectionDetails}}` (no `id`, per the unbind example). Live-verified the shape end-to-end: OLD `{connectionId}` → `InvalidInput` (rejected); NEW `connectionBinding` → `BindModelHasNoBindableConnections` (a SEMANTIC response — the shape is accepted and processed; that particular Direct Lake model simply has no gateway-reboundable cloud data source). The full happy-path bind needs an import/DirectQuery model whose data-source `path` matches the connection's `connectionDetails.path`. Pure `build_bind_connection_body(connection_id, conn_json)` (bind vs unbind, requires `connectionDetails`) is unit-tested; the bind/unbind dry-run e2e are live-gated because the body is built from a live connection GET.
- **`semantic-model evaluate-measure` (the fabio equivalent of semantic-link's `fabric.evaluate_measure`) — live-confirmed**: Evaluates one or more measures over the Power BI `executeQueries` endpoint WITHOUT the caller hand-writing DAX. `build_evaluate_measure_dax(measures, group_by, top)` (pure, unit-tested): no group-by → `EVALUATE ROW("<m>", [<m>], …)` (single-row total); with group-by → `EVALUATE SUMMARIZECOLUMNS(<cols>, "<m>", [<m>], …)`; `--top <n>` → `EVALUATE TOPN(<n>, SUMMARIZECOLUMNS(…), [<first measure>], DESC)`. Group-by column specs are qualified by `qualify_column`: `Table.Column` → `'Table'[Column]` (single quotes in the table name doubled), an already-bracketed `'T'[C]`/`T[C]` passed through. The result rows come back with DAX-bracketed keys (`Sales[Region]`, `[Total Amount]`) — same shape as `query`. Read-only (`mutates:false`, `returns:list`). The DAX execution was refactored out of `query` into a shared `execute_dax_and_render` reused by both. Live-verified on a generated Direct Lake model over a Sales(Region,Amount) table with a `Total Amount = SUM(Sales[Amount])` measure: grouped → `[{Sales[Region]:"EU", [Total Amount]:130}, {…"US"…:50}]`, no group-by → `[{[Total Amount]:180}]`, `--top 1` → EU only. `--measure` is `required` (clap rejects a missing measure before the handler).
- **`semantic-model add-perspective` / `delete-perspective` / `add-perspective-member` / `remove-perspective-member` / `list-perspectives` (perspectives via definition read-modify-write) — live-confirmed**: Perspectives (filtered model views for different audiences) live in `definition/perspectives/<name>.tmdl` and are `ref`-ed from `model.tmdl` (`ref perspective <name>`). The file is a nested tree: `perspective <name>` → `perspectiveTable <T>` → `perspectiveColumn <C>` / `perspectiveMeasure <M>` / `perspectiveHierarchy <H>` (ground-truthed). fabio parses that tree into a small model, edits, and re-renders (like translations/cultures). `add-perspective --name` creates the file + the `ref perspective` line; `delete-perspective` removes both. `add-perspective-member --perspective --table [--column|--measure|--hierarchy]` adds a whole table (no member flag) or one of its members; `remove-perspective-member` removes a member or the whole table. `list-perspectives` is READ-ONLY (`mutates:false`) → `[{name, tableCount}]`. In `model.bim` it's `model.perspectives[].tables[].{columns,measures,hierarchies}[]`. All mutations OVERWRITE the definition → `mutates:true`/`destructive:true`, `--dry-run` guarded; duplicate → `CONFLICT`, missing perspective → `NOT_FOUND`, more than one of `--column`/`--measure`/`--hierarchy` → `INVALID_INPUT`. Pure helpers (`parse_perspective`, `render_perspective`, `remove_member`, plus the `model.bim` variants) in `src/commands/semantic_model/perspectives.rs` are unit-tested; the add → add-member → list → verify → remove-member → delete loop is live-validated (`semantic_model_perspective_lifecycle`).
- **`semantic-model add-function` / `update-function` / `delete-function` / `list-functions` (DAX user-defined functions / UDFs via definition read-modify-write) — live-confirmed**: DAX UDFs (a preview feature) live in `definition/functions.tmdl` as top-level `function <name> = <DAX>` blocks (ground-truthed; the UDF DAX uses the new `(x: INT64) => RETURN x + 1` lambda syntax). Like `expressions.tmdl`, the file is NOT `ref`-ed in `model.tmdl` (auto-discovered), created on the first add / removed when empty. **UDFs REQUIRE model `compatibilityLevel` ≥ 1702** (the current default fabio models are 1604) — `add-function` therefore BUMPS `database.tmdl`'s `compatibilityLevel` to 1702 when it is lower (`ensure_compat_level`; the `model.bim` path bumps the top-level `compatibilityLevel`), live-verified that `updateDefinition` accepts the raised level. `add-`/`update-function --name --expression <DAX UDF>`; `list-functions` is READ-ONLY (`mutates:false`) → `[{name}]`. In `model.bim` it's `model.functions[].{name, expression}`. **`decl_name` in `tmdl.rs` had to learn `function`** (inline `= <DAX>`). All mutations OVERWRITE the definition → `mutates:true`/`destructive:true`, `--dry-run` guarded; duplicate → `CONFLICT`, unknown → `NOT_FOUND`. Pure helpers (`render_function_block`, `ensure_compat_level`, `function_span`, `parse_functions`, `remove_/replace_function_block`, plus the `model.bim` variants) in `src/commands/semantic_model/functions.rs` are unit-tested; the create(1702) → add → list → verify → delete loop is live-validated (`semantic_model_function_lifecycle`, gated on the DAX-UDF preview).
- **`semantic-model add-expression` / `update-expression` / `delete-expression` / `list-expressions` (named expressions / Power Query parameters via definition read-modify-write) — live-confirmed**: Named expressions (shared M queries and Power Query PARAMETERS) live in `definition/expressions.tmdl` as top-level `expression <name> = <M>` blocks (ground-truthed). Like `relationships.tmdl`, the file is NOT `ref`-ed in `model.tmdl` (auto-discovered), and is created on the first add / removed when the last expression is deleted (`upsert_part`/`remove_part`). A Power Query PARAMETER is a named expression whose M is `"<value>" meta [IsParameterQuery=true, Type="<Text|Number|Logical|DateTime>", IsParameterQueryRequired=true]` — fabio's `add-`/`update-expression` accept EITHER `--expression <raw M>` (full control) OR `--parameter-value <val> [--parameter-type]` (builds the parameter M via `build_parameter_m`). `list-expressions` is READ-ONLY (`mutates:false`) and flags each as `isParameter` by scanning for the `IsParameterQuery=true` marker → `[{name, isParameter}]`. In `model.bim` it's `model.expressions[].{name, kind:"m", expression}`. **`decl_name` in `tmdl.rs` had to learn `expression`** (its top-level decl carries an inline `= <M>`, so the name is split on `=`). All mutations OVERWRITE the definition → `mutates:true`/`destructive:true`, `--dry-run` guarded; duplicate → `CONFLICT`, unknown → `NOT_FOUND`. Pure helpers (`build_parameter_m`, `resolve_m`, `render_expression_block`, `expression_span`, `parse_expressions`, `remove_/replace_expression_block`, plus the `model.bim` variants) in `src/commands/semantic_model/expressions.rs` are unit-tested; the create → add(parameter) → list → update-value → verify → delete loop is live-validated (`semantic_model_expression_lifecycle`). NOTE this is DISTINCT from the existing `update-parameters` (a Power BI API call that sets a deployed model's datasource parameter VALUES at runtime) — `add-expression` authors the parameter DEFINITION.
- **`semantic-model add-calculation-group` / `delete-calculation-group` / `add-calculation-item` / `update-calculation-item` / `delete-calculation-item` / `list-calculation-groups` (calculation groups via definition read-modify-write) — live-confirmed**: A calculation group is a SPECIAL table: a `calculationGroup` block (indent 1, NO name — the group name IS the table name) of `calculationItem <name> = <DAX>` entries (indent 2), a single string column (`sourceColumn: Name`), and a `partition <name> = calculationGroup`. THREE model-level prerequisites were ground-truthed live: (1) the calc-group table MUST have a partition whose source type is `calculationGroup` (not `m`/`calculated`); (2) the model MUST set `discourageImplicitMeasures` (TMDL: a bare `discourageImplicitMeasures` line at indent 1 in `model.tmdl`; `add-calculation-group` inserts it via `ensure_model_flag`) — without it the API rejects with "property DiscourageImplicitMeasures must be set to true"; (3) the table is `ref`-ed in `model.tmdl` like any table. `add-calculation-group --name [--column-name]` synthesizes the whole table + the flag + the ref. `add-calculation-item --group --name --expression [--ordinal]` inserts a `calculationItem` INSIDE the `calculationGroup` block (via `calc_group_span`, not `insert_table_child_lines` — items are grandchildren, indent 2); `update-calculation-item --group --name --expression [--ordinal]` replaces an item's DAX in place (preserving scalar props like `ordinal:`); `delete-calculation-item` removes it; `delete-calculation-group` deletes the whole table + ref. `list-calculation-groups` is READ-ONLY (`mutates:false`) → `[{name, itemCount, items}]`. In `model.bim` it's `table.calculationGroup.calculationItems[]` + `model.discourageImplicitMeasures`. **`decl_name` in `tmdl.rs` had to learn `calculationItem`** (its decl carries an inline `= <DAX>`, so the name must be split on `=` like measure/column/partition). All mutations OVERWRITE the definition → `mutates:true`/`destructive:true`, `--dry-run` guarded; duplicate → `CONFLICT`, unknown → `NOT_FOUND`. Pure helpers (`render_calc_group_table`, `ensure_model_flag`, `calc_group_span`, `add_calc_item_tmdl`, `delete_calc_item_tmdl`, `calc_item_names`, plus the `model.bim` variants) in `src/commands/semantic_model/calc_groups.rs` are unit-tested; the create → add-group → add-item×2 → list → verify → delete-item → delete-group loop is live-validated (`semantic_model_calculation_group_lifecycle`).
- **Calc-group partition MUST be `mode: import` on a Direct Lake model (bug fix)**: `add-calculation-group` was completely broken on any **Direct Lake** model (`defaultMode: directLake`, e.g. every `semantic-model generate`-produced model). The calc-group table inherits the model default mode, and its `= calculationGroup` partition is NOT a Direct Lake source-entity partition, so `updateDefinition` rejected it with `Dataset_Import_FailedToImportDataset … Direct Lake Table '<name>' must have an exactly one partition of source entity`. Fix: `render_calc_group_table` now emits the partition as `partition <name> = calculationGroup` + `mode: import`, which overrides the model default so the metadata-only calc-group table is accepted. On an import model `import` is already the default, so the explicit mode is a harmless no-op — the same code path works on both. Found reproducing the calculation-group tutorial against a generated Direct Lake model; before the fix `add-calculation-group` failed on it 100% of the time, after the fix the full add-group → add-item → list loop succeeds live (`[{name:"TimeIntel2", itemCount:1, items:["Current"]}]`). Regression-guarded by the `render_group_table_shape` unit test (asserts `mode: import`).
- **`semantic-model add-partition` / `update-partition` / `delete-partition` / `list-partitions` (table partitions via definition read-modify-write) — live-confirmed**: Partitions define a table's data source and live inside `definition/tables/<T>.tmdl` as `partition <name> = <mode>` blocks (`= m` with a Power Query `source = <M>`; `= calculated` with `mode: import` + `source = <DAX>`; `= calculationGroup`/`= entity` for special tables). A table can carry SEVERAL partitions (e.g. incremental-refresh ranges). `add-partition --table --name (--m <M> | --dax <DAX>)` inserts the block via the shared `insert_table_child_lines`; `update-partition` replaces the partition's `source` (and mode) in place, preserving other property lines; `delete-partition` removes it but REFUSES to delete the last partition (`INVALID_INPUT` — a table must keep ≥1 partition, else `updateDefinition` rejects the table); `list-partitions [--table]` is READ-ONLY (`mutates:false`) → `[{table, name, mode}]`. A single-line source is emitted inline (`source = <expr>`); a multi-line source becomes an indented block (`source =` then indent-3 lines). In `model.bim` the same data is `table.partitions[].source.{type,expression}`. All mutations OVERWRITE the definition → `mutates:true`/`destructive:true`, `--dry-run` guarded; duplicate → `CONFLICT`, unknown → `NOT_FOUND`. Pure helpers (`build_partition_lines`, `update_partition_tmdl`, `delete_partition_tmdl`, `parse_partitions_tmdl`, plus the `model.bim` variants) in `src/commands/semantic_model/partitions.rs` are unit-tested; the create → add(2nd) → list → update-source → verify → delete → last-partition-guard loop is live-validated (`semantic_model_partition_lifecycle`).
- **`semantic-model add-hierarchy` / `delete-hierarchy` / `list-hierarchies` (user drill-down hierarchies via definition read-modify-write) — live-confirmed**: User hierarchies live INSIDE a table's `definition/tables/<T>.tmdl` as a `hierarchy <name>` child block (indent 1) with ordered `level <name>` (indent 2) / `column: <col>` (indent 3) entries (ground-truthed). `add-hierarchy --table --name --level ... [--level ...]` inserts the block via the shared `insert_table_child_lines` (after the table's scalar properties; `hierarchy ` is already a recognized child-object keyword). Each `--level` accepts `Column`, `Name=Column`, or `Name:Column` (level name defaults to the column name). `delete-hierarchy` removes the block via the shared `child_span`; `list-hierarchies [--table]` is READ-ONLY (`mutates:false`) and returns `[{table, name, levelCount, levels}]`. In `model.bim` the same data lives under `table.hierarchies[].levels[]` (with `ordinal`). All mutations OVERWRITE the definition → `mutates:true`/`destructive:true`, `--dry-run` guarded; duplicate → `CONFLICT`, unknown → `NOT_FOUND`. Pure helpers (`parse_level_spec`, `build_hierarchy_lines`, `parse_hierarchies_tmdl`, `delete_hierarchy_tmdl`, plus the `model.bim` variants) in `src/commands/semantic_model/hierarchies.rs` are unit-tested; the create → add → list → verify → delete loop is live-validated (`semantic_model_hierarchy_lifecycle`). **NOTE**: adding these pushed the `semantic-model` dispatch `match` over clippy's `large_stack_frames` threshold (~512 KB) — the granular-authoring arms were split into a second `execute_authoring` function that `execute` delegates to via a catch-all arm (each function's frame is now well under the limit).
...
- **`semantic-model add-table` / `delete-table` / `rename-table` (table lifecycle via definition read-modify-write, with cascade) — live-confirmed**: Tables are `definition/tables/<name>.tmdl` files, each `ref`-ed from `model.tmdl` (`ref table <name>`). `add-table --name --expression <DAX>` creates a CALCULATED table — `table <name>` + `partition <name> = calculated` / `mode: import` / `source = <DAX>` — plus the `ref table` line. It needs NO explicit columns: **Fabric AUTO-INFERS a calculated table's columns** (ground-truthed — a calculated table pushed with no columns came back from getDefinition with an inferred `column n` carrying `isNameInferred` + `sourceColumn: [n]`). `rename-table --new-name` does THREE things: rewrites the `table <old>` declaration, MOVES the part to the new file path (`tables/<old>.tmdl` → `tables/<new>.tmdl`), and updates the `ref table` in `model.tmdl` (DAX/relationship references are NOT rewritten — documented). `delete-table --name` removes the table file + its ref AND **CASCADES**: it removes every relationship in `relationships.tmdl` referencing the table (`remove_relationships_referencing_table` in relationships.rs) and strips `tablePermission <table>` lines from all role files (`cascade_remove_table_from_roles` in roles.rs) — this cascade is REQUIRED because `updateDefinition` rejects a model with a relationship pointing at a deleted table. The removed relationship ids + affected roles are reported in the output (and previewable via `--dry-run`). All three OVERWRITE the definition → `mutates:true`/`destructive:true`, `--dry-run` guarded; duplicate name → `CONFLICT`, unknown table → `NOT_FOUND`. Pure helpers (`render_calculated_table`, `rename_table_decl`, plus the `model.bim` variants which cascade `model.relationships[]`) in `src/commands/semantic_model/tables.rs` are unit-tested; the seed-relationship → add(calculated) → rename(file+ref) → verify → delete(+cascade) loop is live-validated (`semantic_model_table_lifecycle`). **`update-table --name [--hidden|--data-category|--description]`** (added later) sets a table's scalar properties in place — `isHidden` (a bare indent-1 line; `--hidden false` removes it), `dataCategory: <cat>` (e.g. `Time` to mark a date table), and the `///` description (indent 0) — dropping and re-inserting the managed scalar props right after the `table` declaration (before its child objects); in `model.bim` it sets `isHidden`/`dataCategory`/`description`. No-op (no flags) → `INVALID_INPUT`. Live-validated (`semantic_model_update_table_lifecycle`).
...
- **Update from Git supports per-item definition options (spec commit `8baaa4c`)**: `UpdateFromGitRequest.options.itemOptionsByLogicalId` is an array of `{logicalId, options}` entries. Every entry requires a UUID logical ID and an options object, and a logical ID may appear at most once. `git pull --item-options '<json>'` validates those constraints locally and can be combined with `--allow-override`; the two values share the same `options` object.
...
- **Data source type: `Fabric` for a Fabric-hosted KQL database (NOT `AzureDataExplorer`)**: An earlier note claimed the `type` field is always `"AzureDataExplorer"`. That is WRONG for a Fabric Eventhouse KQL database — the portal (and a working authored definition) uses `type:"Fabric"` with `databaseItemId` (the KQL DB's item GUID) + `databaseItemName` (its display name), NOT `databaseName`. Authoring a data source as `type:"AzureDataExplorer"`/`type:"KustoDatabase"` with a `databaseName` is rejected by `updateDefinition` with `InvalidArtifactDefinitionFormat` for a Fabric-hosted KQL database. `type:"AzureDataExplorer"` + `databaseName` remains valid only for an external Azure Data Explorer cluster.
- **Database name is `databaseItemName` on a Fabric data source (run bug)**: `kql-queryset run` originally read only `dataSource.databaseName`; a `type:"Fabric"` data source has no `databaseName` (the name is `databaseItemName`), so the name resolved to empty and Kusto silently fell back to `NetDefaultDB`, failing every run with `BadRequest_EntityNotFound: Entity ID 'NetDefaultDB'`. Fixed to read `databaseName` OR `databaseItemName`.
- **clusterUri for Fabric Eventhouse**: Uses the Kusto query URI format `https://<id>.<region>.kusto.fabric.microsoft.com`. This is the same URI used for direct KQL database queries. For `add-tab`, fabio resolves it from the KQL DB's `properties.queryServiceUri` (GET `/kqlDatabases/{id}`).
...
- **`add-tab` authoring helper (read-modify-write)**: `kql-queryset add-tab` builds a query tab bound to a Fabric KQL database without hand-crafting `RealTimeQueryset.json`. It GETs the KQL DB for `queryServiceUri`/`displayName`, decodes the current definition (empty `{}` → fresh `queryset` skeleton with `version:"1.0.0"`), reuses an existing `type:"Fabric"` data source matching the `databaseItemId` (else appends one with a new UUID), appends `{id, title, content, dataSourceId}` to `tabs[]`, and pushes the whole definition back via `updateDefinition`. Additive (existing tabs preserved), so `mutates:true`/`destructive:false`; the dry-run guard fires before any network call.

## GraphQL API Behaviors Discovered
- **Wiring to a WAREHOUSE source (`sourceType:"DataWarehouse"`, verified live)**: In addition to the SQL-Database path (which binds to the DB's sibling SQL analytics endpoint with `sourceType:"SqlAnalyticsEndpoint"`), a GraphQL API can bind directly to a **Warehouse**: the `graphql-definition.json` datasource uses `sourceType:"DataWarehouse"` with `sourceItemId` = the WAREHOUSE item id itself (a warehouse IS its own SQL endpoint — no sibling item). Live-validated end-to-end: `graphql-api create` → `update-definition` with `{datasources:[{sourceItemId:<warehouseId>, sourceWorkspaceId, sourceType:"DataWarehouse", objects:[{graphqlType:"Products", sourceObject:"dbo.Products", sourceObjectType:"Table"}]}]}` → the server auto-generates the schema → `graphql-api query --gql '{ products { items { ProductId Name Price } } }'` returns the rows. The bound object still MUST have a primary key (else `DataSourceMetadataNotFoundKeyFieldsEmpty`), and the generated query field is a **Connection** type: `products { items { … } }`, NOT `products { … }` (which errors `The field 'ProductId' does not exist on the type 'ProductsConnection'`). NOTE Fabric Warehouse rejects an inline `PRIMARY KEY` in `CREATE TABLE` (`24584: The PRIMARY KEY keyword is not supported in the CREATE TABLE statement in this edition`) — add it with a separate `ALTER TABLE … ADD CONSTRAINT … PRIMARY KEY NONCLUSTERED (col) NOT ENFORCED`.
...
- **Wiring a GraphQL API to a SQL source (reverse-engineered live reproducing the SQL DB E2E tutorial, Aug 2026)**: To connect the API to data you push a `graphql-definition.json` part (the datasource BINDING) — the server then AUTO-GENERATES `schema.graphql` from the bound objects. Pushing a lone `schema.graphql` fails `ALM.MissingDefinitionParts: Missing ALM definition file graphql-definition.json`. Exact object shape (each `datasources[].objects[]` entry): `{graphqlType, sourceObject:"schema.table", sourceObjectType:"Table"|"View"}` — omitting these fails `ALM.InvalidDefinitionMissingRequiredProperties: … 'graphqlType', 'sourceObject', 'sourceObjectType'`. **`sourceType` accepts `SqlAnalyticsEndpoint`** (verified) for a lakehouse SQL endpoint, a warehouse, AND a Fabric SQL database — for a SQL database, `sourceItemId` is the DB's AUTO-CREATED SQL analytics endpoint item id (the sibling `SQLEndpoint` item, provisioned asynchronously a minute after the DB), NOT the `SQLDatabase` item id; **`sourceType:"SqlDatabase"` is REJECTED** (`ALM.InvalidDefinitionInvalidPropertyType` on `sourceType`). **The bound object MUST have a primary key** — a keyless view fails `DataSourceMetadataNotFoundKeyFieldsEmpty` (the tutorial's aggregate view can't be exposed; a keyed table can). The object's SCHEMA must already be mirrored to the SQL analytics endpoint (Fabric SQL DB → SQL analytics endpoint mirroring; schema syncs in ~1 min, data shortly after). The generated query field is the pluralized/camelCased type (`Widget` → `query { widgets { items { WidgetID Name Qty } } }`). fabio's `graphql-api update-definition --file` was FIXED to accept a full definition envelope (via `definition_spec::build_update_definition_body`) — previously it always wrapped the file as a single `schema.graphql`, so it could NOT push a `graphql-definition.json` binding and thus could not wire an API to a source. Live-validated end-to-end: create SQL DB → keyed table → mirror → wire → `graphql-api query` returns the rows (`graphql_api_wire_to_sql_database_and_query`).

## Warehouse API Behaviors Discovered
- **A zero-row TDS SELECT is a result set, not "no result set" (rendering bug fixed)**: `queries-history --label X` (and every TDS list/insight command) misrendered a legitimately-empty result as the scalar `{"data":{"rows_affected":0,"message":"…no result set returned."}}` instead of `{"data":[],"count":0}`, because `execute_and_render_sql` branched on `all_rows.is_empty()`. The correct discriminator is **whether the driver returned a result set at all**: `get_current_resultset()` returns `Some(rs)` with column metadata for ANY `SELECT` (even 0-row), and `None` for DDL/DML (`CREATE`/`INSERT`/`UPDATE`) — so `columns` is non-empty for a SELECT and empty for DDL/DML. Fixed via `tds_utils::produced_result_set(&columns)`. Verified live reproducing the Data Warehouse "data clustering" tutorial: `CLUSTER BY` CTAS + `OPTION (LABEL='Regular'|'Clustered')` labeled queries, then `queries-history --label Regular` → `{"data":[],"count":0}` (was the scalar message); a non-empty SELECT is unchanged; `UPDATE … WHERE 1=0` still returns the no-result-set message. Note: `queryinsights.exec_requests_history` populates asynchronously (up to ~15 min after the first queries on a fresh warehouse) — it briefly returns `Invalid object name 'queryinsights.exec_requests_history'`, which is initialization lag, not a fabio bug.
- **`create --collation` sets the warehouse collation (verified live)**: `CreateWarehouseRequest.creationPayload.collationType` sets the database collation at create time. Valid values: `Latin1_General_100_BIN2_UTF8` (case-sensitive/binary, the default) and `Latin1_General_100_CI_AS_KS_WS_SC_UTF8` (case-insensitive, accent-sensitive). `warehouse create --collation Latin1_General_100_CI_AS_KS_WS_SC_UTF8` → live-verified via `SELECT DATABASEPROPERTYEX(DB_NAME(),'Collation')` that the new warehouse's DB collation is the requested value. Create-time only (collation can't be changed after). Optional; omitting it uses the default case-sensitive collation.
- **`statistics-update`/`statistics-delete` were completely broken — they ignored the OWNING table (found & fixed live)**: A statistic is a per-TABLE object, so `UPDATE STATISTICS <table> (<stat>)` and `DROP STATISTICS <schema>.<table>.<stat>` BOTH require the owning table name — but `warehouse statistics-update`/`statistics-delete` (and the identical `sql-database` pair) emitted `UPDATE STATISTICS [<name>]` / `DROP STATISTICS [<name>]`, treating the STATISTIC name as a TABLE name. Live result: `statistics-update --name stat_id` → `Table 'stat_id' does not exist`; `statistics-delete --name stat_id` → a syntax error (DROP needs `object.statistic`). Both commands take only `--name` (no `--table`), so the fix resolves the owning table at runtime the same way `statistics-show` already did — via `sys.stats s JOIN sys.tables t ON s.object_id = t.object_id WHERE s.name = N'<name>'` — then runs the DDL through `sp_executesql` (the object name can't be a variable in `UPDATE`/`DROP STATISTICS`), with `RAISERROR` when the statistic isn't found. Shared builders `tds_utils::build_update_statistics_sql`/`build_drop_statistics_sql` (unit-tested for table-resolution + `'`→`''` escaping) back all four commands. Live-verified end-to-end on a warehouse: create `stat_id2` on `t.id` → `statistics-update` → `updated` → `statistics-delete` → `deleted` → gone from `statistics-list`. (`sql-database` uses the byte-identical shared builder.)
- **Restore points — request-body + rendering bugs found & fixed (verified live against the spec + tenant)**: (1) `create-restore-point` AND `update-restore-point` sent `{restorePointLabel}`, but the API's `Create`/`UpdateRestorePointRequest` is `{displayName, description}` — the label field is NOT in the schema and was silently ignored, so `--name` did nothing (every restore point was named the default "Restore point", live-confirmed). Both fixed to send `displayName` (+ a new `--description`). The create returns `201` with the `RestorePoint` OR `202` (async) with an EMPTY body; fabio now uses `poll:true` and, if the result lacks an `id`, re-fetches the list and returns the newest restore point (max id — ids are creation-time epochs) so an agent always gets an id back (previously it returned `{"data":null}`). (2) `restore-to-point` (`POST /restorePoints/{id}/restore`) takes **NO request body** — only the restore-point id in the path (restore is IN-PLACE). fabio was REQUIRING a bogus `--name` and sending `{restoreToWarehouseName}` the server ignores; removed `--name`, now sends an empty body and reports `{status:"restored"}` on the empty async response. (3) `list-restore-points` rendered a `restorePointLabel`/`createdDateTime` column pair that don't exist in the response (always empty) — fixed to the real fields `displayName`/`creationMode`/`description`. The restore-point object shape is `{id (epoch-ms string), displayName, description, creationMode ("UserDefined"), creationDetails:{eventDateTime, eventInitiator}}`. NOTE the restore operation itself needs an elevated tenant scope (a device-code token can hit `FORBIDDEN: InsufficientScopes`), but the create/update/label/list/response fixes are fully live-verified.
...
- **Date/time columns via TDS render as proper ISO-8601 (fixed — was "N days since 0001-01-01")**: TDS temporal columns arrive from the mssql-rs crate as raw internal representations (a DATE is a day-count since 0001-01-01; the crate's `time_nanoseconds` field is actually **100-ns ticks**, not nanoseconds; DATETIMEOFFSET stores the datetime2 in UTC + a separate offset). fabio previously emitted these raw (`DATE` → `"739630 days since 0001-01-01"`; legacy `DATETIME` used a naive `1900 + days/365` approximation that produced WRONG dates; `DATETIME2`/`DATETIMEOFFSET`/`SMALLDATETIME`/`TIME` all showed raw day counts or 100×-too-small times). Now `column_value_to_json` (`tds_utils.rs`) converts every temporal type with chrono to readable ISO-8601: DATE → `2026-01-15`; DATETIME2 → `2026-01-15T14:30:45.123456700` (multiplies the 100-ns ticks by 100); DATETIME/SMALLDATETIME base off 1900-01-01; DATETIMEOFFSET adds the offset back to show the LOCAL time (`2026-01-15T14:30:45.000000000+05:30`). Pure helpers `ce_date`/`date_1900`/`time_from_nanos`/`fmt_datetime2`/`fmt_datetimeoffset` are unit-tested; all types live-validated over `warehouse query`. Affects EVERY TDS result (warehouse/sql-database/sql-endpoint/lakehouse). NOTE chrono's fractional-second format supports only `%.3f`/`%.6f`/`%.9f` — `%.7f` panics.
- **Date columns via TDS (legacy note, superseded above)**: historically came through as "N days since 0001-01-01"; now converted client-side.
- **Time-travel `FOR TIMESTAMP AS OF` literal needs 3 fractional-second digits (tutorial-reproduction finding)**: Warehouse time travel (`SELECT … FROM t OPTION (FOR TIMESTAMP AS OF '<datetime>')`) requires the datetime literal to use millisecond (3-digit fractional) precision, e.g. `'2026-08-07T19:03:00.770'`. A full 7-digit ISO-8601 literal (T-SQL `CONVERT(..., 126)`, `SYSUTCDATETIME()`) fails with `Sql Error 241: Conversion failed when converting date and/or time from character string`. Agents composing a time-travel query via `warehouse query` should truncate the captured timestamp to milliseconds. RLS + time travel are otherwise fully reproducible over `warehouse query` (live-validated: RLS filter policy ON→caller's rows only, OFF→all rows; time travel returns the pre-DELETE row count).
...
- **sys.dm_exec_requests columns on Fabric**: Does NOT have `login_name` column — that's in `sys.dm_exec_sessions`. Available columns include: `session_id`, `status`, `command`, `start_time`, `total_elapsed_time`, `sql_handle`, `plan_handle`. Filter `WHERE status != 'background'` to exclude internal TASK MANAGER sessions.
  - **queryinsights schema views**: `queryinsights.frequently_run_queries` (columns: `last_run_command`, `number_of_runs`, `avg_total_elapsed_time_ms`, `min_run_total_elapsed_time_ms`, `max_run_total_elapsed_time_ms`, `number_of_successful_runs`, `query_hash`), `queryinsights.long_running_queries` (columns: `last_run_command`, `number_of_runs`, `median_total_elapsed_time_ms`, `last_run_total_elapsed_time_ms`, `last_run_start_time`, `query_hash`), `queryinsights.exec_requests_history` (columns include `command`, `status`, `label`, `total_elapsed_time_ms`, `allocated_cpu_time_ms`, `data_scanned_remote_storage_mb`, `data_scanned_memory_mb`, `data_scanned_disk_mb`, `login_name`, `start_time`, `end_time`, `row_count`, `query_hash`, `distributed_statement_id`). All views work on Warehouse, Lakehouse SQL Endpoint, AND Fabric SQL Database (same `queryinsights` schema).
  - **`queries-history` surfaces `label` + data-scanned/CPU columns and supports a `--label` filter (verified live)**: `queries-history` (warehouse/sql-endpoint/lakehouse/sql-database) selects the query `label` and performance columns (`allocated_cpu_time_ms`, `data_scanned_remote_storage_mb/_memory_mb/_disk_mb`) and filters `WHERE label = N'...'` when `--label` is passed. Reproduces the Data Warehouse "data clustering" tutorial's effectiveness check: a query on a plain table tagged `OPTION (LABEL = 'Regular')` vs the `CLUSTER BY` table tagged `OPTION (LABEL = 'Clustered')`, then `queries-history --label Clustered` / `--label Regular` compares them. Live-verified the clustered query used less CPU (87 ms vs 151 ms) and the filter returned exactly the tagged rows. **Query Insights lag**: `queryinsights.*` views are provisioned ASYNCHRONOUSLY — on a brand-new warehouse they don't exist for up to ~10-15 min after the first queries, so `queries-history` (old and new alike) returns `Invalid object name 'queryinsights.exec_requests_history'` until then; not a fabio bug.
...
- **New commands added**: `fabio eventstream add-source` and `fabio eventstream add-destination` — high-level helpers that fetch current definition, merge in the new node, auto-create default streams, and push the updated definition. Simplifies agent workflow vs. manually crafting full definition JSON.
 - **`resume`/`resume-source`/`resume-destination` require a `startType` body — the commands were broken (found & fixed live)**: `POST .../resume` (and the source/destination variants) take a `DataSourceStartRequest` body that REQUIRES `startType` (enum `Now`/`WhenLastStopped`/`CustomTime`, with an optional `customStartDateTime` for `CustomTime`). All three fabio resume commands posted an EMPTY `{}` body, so every call failed — live-confirmed: `EventStreamBadWebRequest … Required property 'startType' not found in JSON`. Fixed: each resume now sends `{"startType":<type>}` via a new `--start-type` flag (default `Now`) + `--custom-start-time` (required for `CustomTime`), built by the shared pure `build_start_request` (validates the enum case-insensitively, requires the timestamp for `CustomTime`). Live-verified `eventstream resume` (default Now) now returns `{"status":"resumed"}` (was the `startType` error). `pause`/`pause-source`/`pause-destination` take NO body (unaffected). Pure `build_start_request` is unit-tested; offline dry-run e2e assert the `startType` body + the `CustomTime` guard.
 - **Sample-data source + event-processor operators (reverse-engineered live reproducing the Eventstream transform tutorial, Aug 2026)**: (1) The `SampleData` source's `properties` MUST carry a `type` naming the dataset — `{name, type:"SampleData", properties:{type:"StockMarket"}}`. `add-sample-source` previously sent `properties:{}` and the API rejected it (`EventStreamBadWebRequest … Required property 'type' not found … Path 'properties'`); it now takes `--sample-type` (Bicycles|YellowTaxi|StockMarket|Buses, normalized). (2) The topology's `operators[]` array (the event-processor transform layer — Filter/ManageFields/Aggregate/GroupBy/Join/Union/Expand) had NO fabio authoring path (`list-components` didn't even enumerate operators, and `add-derived-stream` writes to `streams[]`, not `operators[]`). Added `eventstream add-operator --name --type --input-node --properties` (type-validated against the 7) which appends `{name, type, inputNodes:[{name}], properties}` to `operators[]`, plus operators in `list-components --category operator`. (3) The **Filter** operator's condition uses `operatorType` (NOT `operator` as the REST-API doc example shows): `{conditions:[{column:{node, columnName, columnPath:[]}, operatorType:"Equals", value:{dataType:"nvarchar(max)", value:"MSFT"}}]}` — live-verified a Filter operator lands (`operators:[("KeepMSFT","Filter")]`, eventstream status `Running`). Pure `normalize_sample_type` + the `OPERATOR_TYPES` list are unit-tested; the add-sample-source→add-operator loop was live-validated end-to-end.
 - **Eventstream `add-operator` gotchas (input-node = the source's DEFAULT STREAM, not the source; DataType enum) — confirmed live**: Two non-obvious requirements an agent hits when adding an operator, both server-validated (fabio passes `--properties` through faithfully and surfaces the exact server error — dry-run shows the built body is correct). (1) **`--input-node` must reference a STREAM node, not a source.** A source (e.g. `add-sample-source --name bikes`) auto-creates a default stream named `<source>-stream` (`get-topology` → `streams:[{name:"bikes-stream", type:"DefaultStream"}]`). Passing `--input-node bikes` fails with `WorkloadUserException: The operator '…' contains invalid input node(s): bikes. Input node names must be valid stream…`; use `--input-node bikes-stream` (and the Filter condition's `column.node` must also be `bikes-stream`). (2) **The Filter condition `value.dataType` enum is `BigInt | Float | Nvarchar(max) | DateTime | Bit | Record | Array | Any`** (from `operatorCommon.json#/definitions/DataType`) — NOT `Number`/`Int`/`String` (`Error converting value "Number" to type '…Operators.DataType'`). For a numeric column use `BigInt` (integers) or `Float`. Live-validated: `add-operator --type Filter --input-node bikes-stream --properties '{"conditions":[{"column":{"node":"bikes-stream","columnName":"No_Bikes","columnPath":[]},"operatorType":"GreaterThan","value":{"dataType":"BigInt","value":"5"}}]}'` → the Filter lands, `get-topology` → `operators:[("filterBikes","Filter", inputNodes:[{name:"bikes-stream"}])]`.
  - **Eventhouse destination: the two `dataIngestionMode` values have DISJOINT property shapes, and the wrong shape fails SILENTLY (live-diagnosed Aug 2026)**: `add-destination --destination-type Eventhouse` is a `--properties` passthrough, and the Eventhouse destination has two modes with completely different required fields (from `GetEventstreamTopology` example): **`ProcessedIngestion`** (the eventstream provisions ingestion itself — the self-contained, REST-authorable path) needs `{dataIngestionMode:"ProcessedIngestion", workspaceId, itemId, databaseName, tableName, inputSerialization}` and goes to `status:"Running"`; **`DirectIngestion`** needs `{dataIngestionMode:"DirectIngestion", workspaceId, itemId, connectionName, mappingRuleName}` — it references a **pre-existing** Kusto data connection + ingestion mapping you must create on the Eventhouse first, and its steady state is `status:"External"`. The trap: supplying `DirectIngestion` with the ProcessedIngestion fields (`tableName`/`inputSerialization`) but no `connectionName`/`mappingRuleName` is accepted by `updateDefinition` and then sits FOREVER in `status:"Warning"` (`connectionName:null, mappingRuleName:null`), ingests **0 rows**, and logs no `.show ingestion failures` — a silent failure. **A purely REST-authored eventstream→eventhouse pipeline DOES ingest** when you use `ProcessedIngestion`: live-verified end-to-end (sample Bicycles source → ProcessedIngestion Eventhouse destination → the eventstream auto-created the `bikes` table and streamed 1110→3120+ real rows with no portal step). This corrected an earlier WRONG "portal-gated" conclusion — it was a mode/property mistake, not a platform limitation. fabio now validates the Eventhouse destination `--properties` per mode BEFORE any network call (`validate_eventhouse_destination` in `eventstream/builder.rs`, unit-tested): a missing/invalid `dataIngestionMode` or a mode's missing required fields → a teaching `INVALID_INPUT` (`hintType: semantic_correction`) with the exact per-mode example, instead of the silent `Warning`. NOTE `get-destination-connection` returning `Connection is not available for the type 'Kusto'` for an Eventhouse destination is EXPECTED (that endpoint returns an Event-Hub-compatible endpoint, only meaningful for `CustomEndpoint`) and is NOT the ingestion signal. **The silent-failure trap is Eventhouse-SPECIFIC** — the `Lakehouse` destination validates its required fields SERVER-side (a missing `deltaTable` → `EventStreamBadWebRequest: Required property 'deltaTable' not found`; an unsupported `schema` on a non-schema lakehouse → `Cannot specify schema name dbo because schema is not supported by the Lakehouse artifact`), so a wrong Lakehouse shape errors loudly at `add-destination` time and needs no client guard. That is why `validate_eventhouse_destination` is scoped to the Eventhouse destination only. (Lakehouse destination shape, live-verified end-to-end streaming 780 rows: `{workspaceId, itemId, deltaTable, inputSerialization}` + optional `schema`/`minimumRows`/`maximumDurationInSeconds`; omit `schema` for a non-schema lakehouse.)
...
- **`create --min-consumption-units` sets always-on minimum consumption (verified live)**: `CreateEventhouseRequest.creationPayload.minimumConsumptionUnits` (a number) keeps the eventhouse always-on at a minimum capacity. Create-time only. `eventhouse create --min-consumption-units 2.25` → the created eventhouse reports `properties.minimumConsumptionUnits: 2.25` (live-verified via `eventhouse show`). Optional; omitting it uses the default (auto-suspend) behavior.
...
- **UPDATE (Aug 2026): exact 5-part definition schema reverse-engineered + load remains portal-gated (reproducing the Graph E2E tutorial)**: A graph model's `getDefinition` returns FIVE JSON parts (+ `.platform`), each carrying a `$schema` under `.../item/graphIndex/definition/` (NOTE: `graphIndex`, not the `graphInstance` that stored graphs echo): (1) **graphType.json** = the graph SCHEMA — `nodeTypes[{alias, labels[], primaryKeyProperties[], properties[{name,type}]}]` and `edgeTypes[{alias, labels[], sourceNodeType:{alias}, destinationNodeType:{alias}, properties?[]}]`; property `type` ∈ `STRING|INT|UINT|BOOL|DOUBLE` (aliases INT64/FLOAT/FLOAT64/BOOLEAN; temporal/date NOT supported as a property type). (2) **dataSources.json** = `dataSources[{name, type:"DeltaTable", properties:{path:"abfss://<ws>@onelake.dfs.fabric.microsoft.com/<lakehouseId>/Tables/<table>"}}]` (stored graphs echo the internal `onelake.pbidedicated.windows.net` host; either is accepted). (3) **graphDefinition.json** = the MAPPING — `nodeTables[{nodeTypeAlias, id:<guid>, dataSourceName, propertyMappings[{propertyName, sourceColumn}]}]` and `edgeTables[{edgeTypeAlias, id:<guid>, dataSourceName, sourceNodeKeyColumns[], destinationNodeKeyColumns[], propertyMappings?[]}]` — an edge's `sourceNodeKeyColumns`/`destinationNodeKeyColumns` are the FK columns IN THE EDGE'S SOURCE TABLE that match the origin/target node's `primaryKeyProperties`. (4) **stylingConfiguration.json** + (5) **graphSettings.json** = portal visual/settings state. `alias` = numeric-string InternalIdentifier; `id` = GUID. The published JSON schemas are at `github.com/microsoft/json-schemas` under `fabric/item/graphIndex/definition/{graphType,dataSources,graphDefinition,stylingConfiguration}` (the `graphInstance` path 404s). **`updateDefinition` ACCEPTS a full, schema-valid nodes+edges definition** (LRO Succeeded, parts persisted) — so fabio CAN author the complete model over REST. BUT the **LOAD is portal-gated**: `refresh-graph` on a REST-authored graph fails IMMEDIATELY (`lastDataLoadingStatus.status: Failed`, queryReadiness stays `None`) — the internal loading infra is only provisioned when the graph is opened/initialized once in the Fabric portal (`graph-model initialize` returns this as a teaching error: "portal-only operation … VersionConfig does not exist until the portal provisions the internal loading infrastructure"). So a graph built entirely via REST is schema-complete but not queryable until a one-time portal init — a Fabric platform limitation, NOT a fabio bug (mirrors the ontology reasoning gate and data-agent publish gate). GQL `execute-query` DOES work against a portal-loaded graph (verified: `MATCH (n) RETURN count(n)` → `{cnt:6}`). fabio's `context schema GraphModel` / `definition_requirements.json` were CORRECTED from the bogus single `GraphModel.json` part to this real 5-part format.
...
- **`create-rule` KQL source: `--eventhouse-id` is actually the KQL DATABASE item id, NOT the eventhouse (tutorial-reproduction finding)**: The MCP `create_rule` tool's `eventhouseItem` source is `{itemId, workspaceId, itemType:"KustoDatabase"}` — so the `itemId` MUST be the KQL DATABASE item id (the database inside the eventhouse), not the eventhouse item id. Passing the eventhouse id fails with the cryptic MCP error `Rule creation failed … Getting cluster for kql database failed` (HTTP 400). Reproduced live while reproducing the Activator tutorial (a "temperature > 20 for medicine packages, email action" rule): `--eventhouse-id <eventhouse>` → cluster-resolution failure; `--eventhouse-id <kqlDatabase>` OR the explicit `--cluster <eventhouse queryServiceUri> + --database <name>` → rule created. Fixes: the flag now carries a `--kql-database-id` visible alias and a corrected help string ("KQL DATABASE item id, NOT the eventhouse id"), and the cryptic "Getting cluster for kql database failed" MCP error is caught and re-emitted with a teaching hint. The tutorial's **property filter** ("only medicine packages") is expressed in the monitored-stream KQL itself (`--kql "DeliveryEvents | where SpecialCareContents == 'Medicine'"`), so a single-condition rule can carry an arbitrary pre-filter — no separate filter flag needed. Full loop live-validated: create-rule → list-rules (`isRunning:true`) → start-rule → stop-rule.
...
- **Tags**: `POST /workspaces/{ws}/applyTags` and `/unapplyTags` with body `{"tags": [...]}` (an array of tag ID UUIDs — the field is `tags`, NOT `tagIds`; see the bug note below).
- **Domain assignment**: `POST /workspaces/{ws}/assignToDomain` with `{"domainId": "<id>"}`. Unassign uses empty body.
- **OneLake settings and access-time tracking**: `GET /workspaces/{ws}/onelake/settings` returns tier, diagnostics, immutability, and optional `lifecycle.accessTimeTracking` (`Enabled` or `Disabled`). `POST .../settings/modifyAccessTimeTracking` requires `{"status":"Enabled|Disabled"}`, takes effect immediately, and requires workspace Admin plus `OneLake.ReadWrite.All`. When enabled, file reads and writes update Azure Storage `LastAccessTime`; metadata-only property, metadata, and tag operations do not. The endpoint can return `OneLakeNotProvisioned` when the workspace has no items.
...
- **applyTags/unapplyTags used the WRONG body field `tagIds` — the "API_ERROR on some tenants" was actually a fabio bug (found & fixed live)**: `POST /workspaces/{ws}/applyTags`, `/unapplyTags`, and the item-level `POST /workspaces/{ws}/items/{id}/applyTags`/`unapplyTags` take an `ApplyTagsRequest`/`ApplyWorkspaceTagsRequest` whose ONLY (required) field is **`tags`** (an array of tag ID UUIDs) — NOT `tagIds`. fabio sent `{"tagIds":[...]}` for ALL FOUR commands, which the API rejects with `InvalidInput: The request has an invalid input`. This was previously mis-documented here as a mysterious "returns API_ERROR on some tenants, root cause unknown" tenant limitation, and an e2e test even SKIPPED on the error — it was a plain field-name bug all along. Live-confirmed: `{tagIds}` → InvalidInput; `{tags}` → success (`{}`/200, tag applied). Fixed all four commands to send `{"tags": [...]}`. Live-validated end-to-end (create tag via admin → `item apply-tags`/`unapply-tags` and `workspace apply-tags`/`unapply-tags` → `applied`/`unapplied`). Offline dry-run e2e assert the body carries `tags` (not `tagIds`), and the workspace e2e no longer skips on the error.
...
- **CMK encryption endpoints (Generally Available as of August 2026)**: Three workspace-scoped encryption endpoints:
...
- **Admin list-workspaces is Generally Available as of August 2026**: `GET /admin/workspaces` is no longer marked Preview. `?include=encryption` adds `encryption.status` and `encryption.keyIdentifier` fields to each workspace in the response. `?encryptionStatus=<status>` filters results — only valid when `include=encryption` is also specified.

## Common Error API Behaviors Discovered

- **Structured error parameters (spec commit `8baaa4c`)**: The common Fabric error object can include a read-only `parameters` array. Each entry may carry `name`, `value`, and `message` strings that provide machine-readable context beyond `error.message`. fabio preserves these entries as `error.parameters` in its JSON error envelope, alongside `moreDetails`, `relatedResource`, `requestId`, and `retriable`.

## Item API Behaviors Discovered

- **Definition option envelopes (spec commit `8baaa4c`)**: Generic item create accepts a top-level `options` object only when `definition` is also present; sending options without a definition is `BadRequest`. Generic `updateDefinition` accepts the same top-level `options` sibling of `definition`. Option names and values are item-type specific. fabio exposes `item create --definition ... --options ...` and `item update-definition --options ...`; both require JSON objects and preserve the standard dry-run preview.
- **Bulk definition and item-move APIs are GA**: `bulkExportDefinitions`, `bulkImportDefinitions`, `bulkMove`, and single-item folder `move` no longer carry Beta/Preview warnings. Bulk export/import no longer accept or require `?beta=true`; child items still move with their parent and cannot be moved independently.
...
- **Bulk operations (all LRO)**:
  ...
  - `POST /workspaces/{ws}/items/bulkImportDefinitions` — imports flat definition parts. Body: `{"definitionParts":[...],"options":{"allowPairingByName":true,"itemOptionsByLogicalId":[{"logicalId":"<uuid>","options":{...}}]}}`. `allowPairingByName` matches an item lacking a logical ID by display name and type. Each per-item entry requires a UUID `logicalId` plus an `options` object, and each logical ID may appear at most once. The former required `?beta=True` parameter was removed at GA.
  ...
  - **Workspace clone**: `fabio workspace clone --source <WS> --dest <WS>` orchestrates bulk export → transform → bulk import. Supports `--allow-pairing-by-name`, `--item-options`, and `--item-types` for selective cloning.
- **External data shares — recipient is a discriminated union; the old shape returns HTTP 500 (found & fixed live)**: CRUD at `/workspaces/{ws}/items/{id}/externalDataShares`. Create body: `{"paths":[...], "recipient":{...}}` where `paths` must start with `Files/` or `Tables/`. The `recipient` is a **discriminated union on `type`** (`CreateExternalDataShareRequest` → `BaseExternalDataShareRecipient`): a **User** recipient is `{"type":"User","userPrincipalName":"<email>","tenantId":"<id>"?}` (email required, tenantId optional); a **ServicePrincipal** recipient is `{"type":"ServicePrincipal","principalId":"<object-id>","tenantId":"<id>"}` (both required). fabio previously sent the OLD shape `{"tenantId","objectId","recipientType"}` (and a bare `{"tenantId"}` for "whole tenant") — **all three are rejected by the deployed API with `InternalServerError: An error occured` (HTTP 500)**, so `item create-external-data-share` was completely broken for every recipient. Live-verified: old User/SP/bare shapes → 500; new `{type,userPrincipalName}` and `{type,principalId,tenantId}` → success (`status: Pending`). There is no longer a "share with the entire tenant" mode — every share targets a specific User (by email) or ServicePrincipal (by id+tenant). Fixed: `create-external-data-share` now takes `--recipient-type User|ServicePrincipal` (default User), `--recipient-email` (User), `--recipient-id` (SP principalId), `--recipient-tenant-id` (required for SP, optional for User); the pure `build_eds_recipient` builds the correct union and enforces the per-type required fields before any network call. Response `recipient` echoes `type`+`userPrincipalName` (User) or `type`+`tenantId`+`principalId`+`recipientObjectId` (SP). Testing requires the tenant setting **`AllowExternalDataSharingSwitch`** ("External data sharing") enabled AND a FOREIGN recipient tenant id (same-tenant is rejected with `InvalidInput: External data sharing in the same tenant is not allowed`). Pure `build_eds_recipient` is unit-tested; both recipient types are live-validated end-to-end via the CLI + offline dry-run regression e2e.
- **External data shares (legacy note, superseded above)**: Accept invitations at `/externalDataShares/invitations/{id}/accept`.
- **`get-invitation` requires a `providerTenantId` query param — the command was broken (found & fixed live)**: `GET /externalDataShares/invitations/{invitationId}` REQUIRES `?providerTenantId=<id>` (the tenant that issued the invitation). fabio omitted it — the URL without the param does not match the route (`NOT_FOUND: The endpoint or resource could not be found`). Fixed: `item get-invitation` gained a required `--provider-tenant-id`. Live-verified: WITH it → the request routes correctly (returns `TenantSwitchDisabled` when the "Users can accept external data shares" tenant setting is off — a semantic gate), vs the old `NOT_FOUND`.
...
- **Identity assignment (`assign-identity`) needs `assignmentType` + `?beta=true` — the command was broken (found & fixed live)**: `POST /workspaces/{ws}/items/{id}/identities/default/assign` associates the caller as the item's managed identity. It has TWO hard requirements fabio was missing: (1) the body `UpdateItemIdentityRequest` REQUIRES `assignmentType` (enum, currently only `"Caller"`); (2) the endpoint is gated behind a REQUIRED `?beta=true` query param. fabio posted an EMPTY `{}` body to the URL WITHOUT `?beta=true`, so every call failed — live-confirmed: no beta → `BetaParameterRequired: The beta query parameter must be set to true`; beta but empty body → `InvalidInput`. Fixed: `assign-identity` now sends `{"assignmentType":"Caller"}` (via a new `--assignment-type` flag defaulting to `Caller`) to `.../assign?beta=true`. Live-verified the corrected shape is accepted (a Lakehouse returns the semantic `InvalidInput: The requesting user is already the identity of the artifact` — a business idempotency response, NOT a shape rejection — proving beta+assignmentType are parsed; a Notebook returns `OperationNotSupportedForItem`, i.e. not all item types support it). Offline dry-run e2e asserts the `assignmentType` body field.
- **Identity assignment (legacy note)**: `POST /workspaces/{ws}/items/{id}/identities/default/assign`.
- **Tags**: `POST /workspaces/{ws}/items/{id}/applyTags` and `/unapplyTags` with `{"tags": [...]}` (field is `tags`, not `tagIds` — see the applyTags bug note above).
...
- **Schemas-enabled lakehouses reject the REST tables/load endpoints (`UnsupportedOperationForSchemasEnabledLakehouse`, verified live)**: For a lakehouse created with `--enable-schemas`, `GET /workspaces/{ws}/lakehouses/{id}/tables` AND `POST .../tables/{table}/load` (the NON-schema paths) both fail HTTP 400 `UnsupportedOperationForSchemasEnabledLakehouse: The operation is not supported for Lakehouse with schemas enabled.` Two fabio consequences: (1) **`lakehouse list-tables` falls back to OneLake enumeration** — it lists the schema directories under `Tables/` then the table directories under each `Tables/<schema>/` via a non-recursive OneLake DFS filesystem-list, returning `{name, schema, type:"Managed", format:"Delta", location}` rows (the `client::list_onelake_dir` helper; the REST path is used first and the fallback only fires on the schemas-enabled error, so non-schema lakehouses are unchanged). (2) **`load-table`/`upload-table` require `--schema`** → they route to `POST .../schemas/{schema}/tables/{table}/load?beta=true` (the schema-aware endpoint, which DOES work). `upload-table` gained `--schema` (parity with `load-table`); when `--schema` is omitted on a schemas-enabled lakehouse, fabio maps the raw API error to a teaching hint pointing at `--schema` (`map_schemas_enabled_error`). **OneLake DFS directory-scoping gotcha**: the item-in-path list shape (`/{ws}/{item}?resource=filesystem&directory=Tables`) does NOT scope `directory` to a subtree — it must be the workspace-scoped shape (`/{ws}?resource=filesystem&recursive=false&directory={item}/Tables`), which BOTH `list_onelake_dir` and (since the `list-files --path` fix) `list_onelake_files` use. **Fabric Spark reads a public/anonymous Azure blob directly** via `wasbs://<container>@<account>.blob.core.windows.net/<path>` with no credential config (used to reproduce the Lakehouse E2E tutorial ingestion from the public WWI sample blob into `Tables/<schema>/...`).
...
- **External libraries**: Export via `GET .../libraries/exportExternalLibraries`. Import via `POST .../staging/libraries/importExternalLibraries`. Remove via `POST .../staging/libraries/removeExternalLibrary` with `{"name":"<name>","version":"<version>"}` (see the removeExternalLibrary bug note below).
- **`remove-staging-library` requires `{name, version}` — the command was broken (found & fixed live)**: `POST .../staging/libraries/removeExternalLibrary` (`RemoveExternalLibrariesRequest`) REQUIRES BOTH `name` AND `version` (the exact library version). fabio sent `{"libraryToRemove":"<name>"}`, which the API rejects with `EnvironmentValidationFailed: Invalid request. Provide the name and version of external library to be deleted.` — and a `{name}`-only body fails the same way (version really is required). Fixed: `remove-staging-library` now sends `{"name":<library_name>,"version":<library_version>}` via a new REQUIRED `--library-version` flag. Live-verified end-to-end: import `fuzzywuzzy==0.18.0` (environment.yml, octet-stream) → `remove-staging-library --library-name fuzzywuzzy --library-version 0.18.0` → `{status:"removed"}`; the wrong/name-only bodies fail with the "Provide the name and version" error. Offline dry-run e2e assert the `name`+`version` body.
...
- **Environment custom live pools**: staging Spark compute accepts optional `customLivePoolSupport` (`Enabled`/`Disabled`) and `customLivePoolSettings`. Settings contain `maxClustersToHydrate` (minimum 1), `clusterIdleTimeout` (ISO 8601 `PT20M` through `PT24H`), and `customLivePoolLifespan` (`PT30M` through `PT24H`). Disabling support retains settings; sending `customLivePoolSettings: null` removes them; omission leaves them unchanged. `instancePool.maxClustersToHydrateLimit` is a read-only, capacity-dependent upper bound. fabio exposes typed flags and preserves unspecified fields with its existing read-merge-write path.

## Mirrored Database API Behaviors Discovered
- **Definition file**: Part path is `mirroring.json`.
- **Open mirroring (push-based, reverse-engineered live reproducing the Open Mirroring tutorial, Aug 2026)**: An OPEN mirrored database (no source connection — you push data files) is created by giving `mirroring.json` the source type **`GenericMirror`** with empty `typeProperties` (target `MountedRelationalDatabase`, `format: Delta`, `defaultSchema`): `{properties:{source:{type:"GenericMirror",typeProperties:{}},target:{type:"MountedRelationalDatabase",typeProperties:{defaultSchema:"dbo",format:"Delta"}}}}` (verified the exact accepted source enum — `Generic`/`OpenMirroring`/`GenericMirroring` are all rejected). `POST /mirroredDatabases` with ONLY a `displayName` (fabio's old `create`) leaves the item with NO definition — `getDefinition` then fails `MirroringDefinitionMissing`, so `create --open-mirroring` posts the item then `updateDefinition`s the GenericMirror definition. A freshly-created item is briefly `MirroredDatabaseNotReady` ("being initialized, retry later"), so the definition push RETRIES (6× / 12 s). **Landing zone** = `https://onelake.dfs.fabric.microsoft.com/<ws>/<mirroredDbId>/Files/LandingZone/<TableName>/` — push a numbered `<n>.parquet` (20-digit zero-padded, `00000000000000000001.parquet` for the initial full snapshot) per table plus a `_metadata.json` (`{"keyColumns":[...]}`) in each table folder; `mirrored-database landing-zone` prints the URL (pure string construction, no API call, `mutates:false`). The landing-zone upload is reachable via fabio's item-generic OneLake uploader — `fabio lakehouse upload --id <mirroredDbId> --dest-path Files/LandingZone/<Table>/<file>` works against a mirrored-database item (verified). Then `mirrored-database start`; Fabric monitors the landing zone and replicates to OneLake Tables. Pure `open_mirroring_definition` is unit-tested; the create(`--open-mirroring`)→get-definition→landing-zone loop is live-validated (`mirrored_database_open_mirroring_lifecycle`).
...
- **Status endpoints are POST, NOT GET (bug fix, verified live)**: `POST .../getMirroringStatus` and `POST .../getTablesMirroringStatus` — both are POST actions (an earlier note here wrongly claimed GET; fabio's `status`/`table-status` used `.get()` and returned `EntityNotFound: The requested resource could not be found` for every call — completely broken). Fixed to POST with an empty body `{}`. `getMirroringStatus` returns `{status}` (`Initialized`/`Running`/`Stopped`/…); `getTablesMirroringStatus` returns a PAGINATED per-table list `{continuationToken, continuationUri, data:[{sourceSchemaName, sourceTableName, status (e.g. Replicating), metrics:{processedBytes, processedRows, lastSyncDateTime}}]}`. `table-status` now renders the `data` array as a proper list (`{data:[…], count}` + continuation token) so agents can filter (e.g. `--query "[?status!='Replicating']"`). Live-verified on an open mirrored database: GET → `EntityNotFound`, POST → `{"status":"Initialized"}`.
- **`warehouse query` resolves a MirroredDatabase SQL endpoint (bug fix, verified live)**: A `MirroredDatabase` exposes its read-only SQL analytics endpoint (over the replicated OneLake Delta tables) at `properties.sqlEndpointProperties.connectionString` (+ `.id` = the sibling SQLEndpoint item id) — the SAME shape as a lakehouse (`mirrored-database show` confirms it). But `warehouse::get_connection_string` only tried warehouse/lakehouse/warehouseSnapshot/mirroredAzureDatabricksCatalog, so `warehouse query --id <mirroredDb>` failed with `Could not determine SQL connection string` (0-copy queryability was unreachable via the smart resolver). Added the `mirroredDatabases` fallback (same precedent as the `MirroredAzureDatabricksCatalog` fix). Live-verified reproducing the open-mirroring tutorial end-to-end: push a Products snapshot parquet → `start` → `warehouse query --id <mirroredDb> --sql "SELECT id,name,amount FROM Products"` returned all 3 mirrored rows. NOTE `sql-endpoint query --id <mirroredDb>` is by design NOT the path — it resolves a SQLEndpoint item's OWN id via `/sqlEndpoints/{id}` and returns `ItemNotFound` for the parent mirrored-DB id; query the mirror via `warehouse query --id <mirroredDb>` (smart resolver) or `sql-endpoint query --id <sqlEndpointProperties.id>`. The resolver's per-shape extraction is now a pure, unit-tested `extract_connection(data, via_sql_endpoint)`.
...
- **Deploy body and per-item options**: The base body is `{"sourceStageId": "<id>", "targetStageId"?: "<id>", "items"?: [...], "note"?: "<text>"}`. `targetStageId` optional (defaults to next stage); `items` optional (defaults to all items). `options.allowCrossRegionDeployment` enables cross-region deployment. Spec commit `8baaa4c` added `options.itemOptionsBySourceItemId`: an array of `{sourceItemId, options}` entries, each with a UUID source item ID and an options object, with each source item ID appearing at most once. fabio exposes these as `--allow-cross-region-deployment` and `--item-options`.
- **Deploy is LRO**: `POST /deploymentPipelines/{id}/deploy` with `poll: true`. May return empty/null response (treated as "accepted").
- **Items array format**: `[{"sourceItemId": "...", "itemType": "Notebook"}]` — identify each deployed item by its source-stage item ID and use PascalCase item types.
...
- **`update` (PATCH) requires `connectivityType` as the discriminator — the command was broken without it (found & fixed live)**: `UpdateConnectionRequest` is a discriminated union on `connectivityType`, and that field is REQUIRED in the PATCH body (it selects which concrete update shape — `UpdateShareableCloudConnectionRequest`, `UpdateOnPremisesGatewayConnectionRequest`, … — applies). `connection update` built `{displayName?, privacyLevel?, credentialDetails?}` WITHOUT `connectivityType`, so every update failed with `API_ERROR: InvalidInput: The request has an invalid input` (live-confirmed on a `ShareableCloud` connection: PATCH without it → InvalidInput; PATCH with `{"connectivityType":"ShareableCloud","privacyLevel":"Organizational"}` → success). Fixed: `update` now GETs the connection first to read its current `connectivityType` and includes it in the PATCH body (the same pattern `gateway update` uses for its required `type` discriminator). The user does NOT pass the type — it's resolved automatically. Pure `build_connection_update_body(connectivity_type, name, privacy_level, credential_type, credentials)` is unit-tested (asserts `connectivityType` is always present); live-verified `connection update --privacy-level None` now succeeds.
...
- **Connection recency (governance signal, from the List Connections API)**: `GET /connections` returns an optional `connectionRecency` object per connection (present for cloud/gateway connections that report recency; ABSENT for older connections and some connectivity types): `{createdDateTime, lastBoundDateTime, lastCredentialUsedDateTime, myLastBoundDateTime, myLastCredentialUsedDateTime}` (all RFC 3339 `YYYY-MM-DDTHH:mm:ssZ`, the `my*` variants scoped to the caller). Semantics: `lastBoundDateTime` = when any item was last LINKED to the connection (NULL on a post-GA connection ⇒ not bound to any item); `lastCredentialUsedDateTime` = when the credentials were last USED by any workload (can lag real time by up to ~30 min — governance, not live monitoring). Linking ≠ usage. **Caveat**: connections created before connection-recency GA (~March 2026) report NULL recency even when actively used, so any NULL-based staleness heuristic MUST gate on `createdDateTime >= a safe cutoff` (fabio uses 2026-05-01 as the default `--created-after`) to avoid false positives. fabio surfaces `LAST BOUND`/`LAST USED` in the `connection list` table (auto-added only when `connectionRecency` is present; `createdDateTime` always in `--output json`) and adds three READ-ONLY governance commands built purely on these signals + role assignments: `connection find-stale` (never-bound / never-used / credentials-unused-since-N-days, gated on `--created-after`), `connection find-duplicates` (groups by `connectionDetails.type` + `path` + `connectivityType` + `gatewayId`, keeps the most-recently-used, emits the rest with a `keepId`; `--match-credential-type` adds `credentialType` to the key), and `connection find-single-owner` (one `roleAssignments` read per connection — no bulk API — flags connections whose ONLY `Owner` role assignment is an individual `User`; skips connections whose role assignments it can't read rather than aborting). Gateways cap at 1,000 connections, so this hygiene keeps headroom. **Owner-deletion myth**: a connection fails when its auth CREDENTIAL can no longer authenticate (e.g. an OAuth credential tied to a deleted user), NOT merely because the owner account was deleted — an SPN-based shared connection keeps working; single individual ownership is still a continuity risk worth fixing by adding a durable Entra-group owner. Cloud-connection visibility is owner-scoped (a gateway admin sees all connections on the gateway).
...
- **Spark monitoring APIs for a Spark application (verified live)**: Per-application diagnostics live under `/workspaces/{ws}/{notebooks|sparkJobDefinitions|lakehouses}/{itemId}/livySessions/{livyId}/applications/{appId}[/{attemptId}]/…`. The `{appId}` (`sparkApplicationId`, e.g. `application_1786046819149_0001`) and `{attemptId}` (`attemptNumber`) come from the Livy session object (`GET .../livySessions/{livyId}`). Endpoints (all GET, read-only): `/advice` → Spark advisor recommendations (array of `{id, name, description, level, source, stageId, jobId, …}`; `[]` for a trivial run); `/resourceUsage` → a timeline object (`{resourceUsageApiVersion, duration, coreEfficiency, idleTime, capacityExceeded, data:{timestamps[], …}}`); `/logs?type=driver|executor|livy&meta=<bool>[&fileName=<f>]` → with `meta=true` returns log-file metadata JSON (driver: `{containerId, nodeId, containerLogMeta:{fileName, length, lastModified, …}}`; livy: `{fileName:"livy.log", length, …}`), with `meta=false` returns the RAW log text (not JSON). **Livy logs use application id `none` and no attemptId** (`.../applications/none/logs?type=livy`); driver/executor logs need a real `appId`. Surfaced by `spark get-advice` / `spark get-resource-usage` / `spark get-logs`.
...
- **Interactive Livy API — create sessions and run code (live-verified Sep 2026)**: DISTINCT from the read-only monitoring API above. The interactive Livy API is per-lakehouse: `…/workspaces/{ws}/lakehouses/{lh}/livyApi/versions/2023-12-01/sessions`. Verified contract: **create** `POST …/sessions {name?, conf?}` → `{id, artifactId}` (POST returns only id+artifactId; `name`/`conf` are reflected on a subsequent GET, but a body `tags`/`sessionTag` came back EMPTY — the HC session-tag mechanism is NOT a simple body field, so fabio exposes generic `--conf` instead of guessing it). **state** `GET …/sessions/{id}` → `{state}` progressing `not_started`→`starting`→`idle`→`busy`→`dead`; a cold session reaches `idle` in ~100s. **statement** `POST …/sessions/{id}/statements {code, kind}` (kind ∈ `pyspark`/`spark`(Scala)/`sql`/`sparkr`) → `{id, state:"waiting"}`, then `GET …/statements/{stmtId}` → `{state:"running"→"available", output}`. **Statement output shape**: `output.status` = `ok`/`error`; for pyspark/scala the result is `output.data["text/plain"]`; for **SQL** the result is NOT text/plain — it is `output.data["application/json"] = {schema:{fields:[…]}, data:[[…]], truncated}`. On error: `output.status="error"` with `ename`/`evalue`/`traceback[]`. **delete** `DELETE …/sessions/{id}` → null. Sessions consume Spark capacity until deleted. Surfaced by `spark create-livy-session`/`run-statement`/`delete-livy-session` and the one-shot `spark run` (create→wait idle→run→output→delete, always cleans up; all waits bounded by `--timeout`, default 300s). Live-proven end to end: `spark run --code "print(sum(range(10)))"` → `text:"45"`; SQL `SELECT 1 AS a,'hi' AS b` → `data["application/json"].data = [[1,"hi"]]`; `raise ValueError('boom')` → exit 1 with `ename:ValueError, evalue:boom`.
- **High-Concurrency (HC) Livy sessions (live-verified Sep 2026)**: A SEPARATE endpoint from the regular interactive sessions: `…/lakehouses/{lh}/livyApi/versions/2023-12-01/highConcurrencySessions`. **create** `POST …/highConcurrencySessions {sessionTag?}` → `{id, state:"NotStarted", sessionTag, …}` — here the `sessionTag` IS reflected (unlike the regular `/sessions` endpoint, where a body `tags`/`sessionTag` came back empty; that is WHY session-packing is not a `/sessions` conf). **poll** `GET …/highConcurrencySessions/{hcId}` → state `NotStarted`→`AcquiringHighConcurrencySession`→`Idle` (reached `Idle` in ~48s live); ONLY once `Idle` does it return the underlying `sessionId` (shared/packed when two HC sessions share a `sessionTag`) and `replId` (isolated per HC session). **statement** `POST …/highConcurrencySessions/{sessionId}/repls/{replId}/statements {code, kind}` — note statements route through the **underlying `sessionId` + `replId`**, NOT the HC id — then `GET …/repls/{replId}/statements/{stmtId}` (same output shape as regular). **delete** `DELETE …/highConcurrencySessions/{hcId}` — uses the **HC id**, not the underlying sessionId. States are Capitalized (`Idle`/`Dead`/`Failed`) vs the regular endpoint's lowercase, so state checks are case-insensitive. fabio exposes HC through the one-shot `spark run --high-concurrency [--session-tag <tag>]`, which hides the hc-id/sessionId/replId plumbing (create HC → wait Idle → run through the REPL → delete by HC id); output carries `highConcurrency: true`.

## Data Pipeline API Behaviors Discovered
- **ORCHESTRATION activities ARE REST-authorable and run end-to-end (live-verified Aug 2026)**: Unlike copy-job's data-MOVEMENT activities (portal-oriented — a hand-authored `Copy` activity is rejected `UnknownError`), a data pipeline's ORCHESTRATION activities can be authored via `update-definition` and executed. Live-proven: a `pipeline-content.json` with a **Notebook** activity — `{"properties":{"activities":[{"name":"RunNotebook","type":"TridentNotebook","dependsOn":[],"typeProperties":{"notebookId":"<nb>","workspaceId":"<ws>"}}]}}` — pushed via `data-pipeline update-definition`, then `data-pipeline run --wait` → `status: Completed` AND the notebook actually executed (it wrote its Delta table to the lakehouse). The Fabric notebook-activity type is **`TridentNotebook`** (not "Notebook"). So agents CAN build orchestration pipelines (invoke notebook / dataflow / child pipeline / etc.) purely via fabio; only the connector-based data-movement (Copy source/sink) needs the portal. `list-instances`/`get-instance` reflect the run (`status:Completed`, `jobType:execute`). **Schedule authoring shape**: `data-pipeline create-schedule --content '{"enabled":true,"configuration":{"type":"Cron","interval":<min>,"startDateTime","endDateTime","localTimeZoneId"}}'` (the config MUST be nested under `configuration`; `--content`/`--file`, NOT `--config`) — live-verified create→list→get→update→delete.
...
- **KQL Database Table Shortcut Behaviors (`CreateTableShortcutRequest` REQUIRES `enableQueryAcceleration`; 6-target subset)**: A KQL database's table shortcuts live at `/workspaces/{ws}/kqlDatabases/{id}/shortcuts` (the `TableShortcuts` API: `GET`=list `{value:[…]}`, `POST`=create, `GET .../{name}`, `DELETE .../{name}`) — a SEPARATE resource from OneLake item shortcuts (`/items/{id}/shortcuts`). The `CreateTableShortcutRequest` body REQUIRES all three of `name`, `enableQueryAcceleration` (bool), and `target` — omitting `enableQueryAcceleration` is rejected by the server. The `target` is a `CreatableShortcutTarget`: a `{<discriminator>: {...}}` object (same shape as a lakehouse shortcut `Target` but WITHOUT the `type` field), and it supports only SIX storage discriminators — `oneLake`, `amazonS3`, `adlsGen2`, `googleCloudStorage`, `s3Compatible`, `azureBlobStorage` — NOT `dataverse`/`externalDataShare`/`oneDriveSharePoint` (those three are valid for lakehouse OneLake shortcuts but not for a KQL table shortcut). `fabio kql-database create-shortcut` builds this typed (reusing the shared `shortcut_target::build_shortcut_target`) and rejects the 3 unsupported types offline (`ensure_kql_target_supported`). Live-verified end-to-end: a typed `OneLake` target pointing at a lakehouse Delta table (`--target-path Tables/sales`) with `--enable-query-acceleration` creates a working shortcut — `GET .../{name}` reflects `enableQueryAcceleration:true`, and `<shortcut> | count` over the Kusto data plane returns the underlying table's rows (count==2). NOTE `list-shortcuts` may briefly show `enableQueryAcceleration:false` for a just-created shortcut while acceleration provisions asynchronously, even though the per-shortcut `GET` already reports the requested `true`. (The pre-existing `bulk-create-shortcuts` on kql-database targets the generic OneLake `/items/{id}/shortcuts/bulkCreate` — a different, item-level shortcut system — not the TableShortcuts API; left unchanged.)
- **T-SQL over a KQL database needs the SQL dialect option (verified live reproducing the RTI tutorial's "Query using T-SQL" step)**: A leading-`SELECT` T-SQL query is REJECTED by Kusto as invalid KQL (`SyntaxException: Expected: ;`) on BOTH `/v1/rest/query` and `/v2/rest/query` UNLESS the request body carries `properties.Options.query_language = "Sql"` AND is sent to `/v1/rest/query` (the T-SQL result comes back in the v1 `{"Tables":[...]}` shape, not the v2 frame array). `fabio kql-database query` now auto-detects a T-SQL query (the query's leading alphabetic run equals `select`, case-insensitively — KQL has no `SELECT` operator, so this is unambiguous; an identifier like `SelectedRows` is NOT misdetected) and routes it to `/v1/rest/query` with the SQL option + v1 parsing; `.`-prefixed management commands still use `/v1/rest/mgmt` and other KQL queries still use `/v2/rest/query` (no regression). Pure `is_tsql_query` in `kql_utils.rs` is unit-tested; live-validated (`kql_database_query_tsql`: `SELECT TOP 2 Region, Amount FROM SalesEvents ORDER BY Amount DESC` returns rows). This applies to every command that runs through `execute_kql` (kql-database query, kql-queryset run, …).
...
- **`create-schedule`/`update-schedule` config must be NESTED under `configuration` — the commands were broken (found & fixed live)**: `CreateScheduleRequest`/`UpdateScheduleRequest` is `{enabled, configuration:{...}}`, where `configuration` (`ScheduleConfig`) REQUIRES `type` (Cron|Daily|Weekly|Monthly), `startDateTime`, `endDateTime`, `localTimeZoneId`, plus per-type fields (Cron→`interval`, Daily→`times[]`, Weekly→`times[]`+`weekdays[]`, Monthly→`recurrence`+`occurrence`+`times[]`). fabio SPREAD the `--config` object at the TOP LEVEL (`{enabled, type, …}`) instead of nesting it, and its help example used nonexistent fields (`expression`/`timezone`), so every schedule create/update failed with `InvalidInput` (live-confirmed: flat → InvalidInput; `{enabled, configuration:{type:"Cron", startDateTime, endDateTime, localTimeZoneId, interval:10}}` → 201 created). Fixed: both commands now set `body["configuration"] = <config>` via the shared pure `schedule_configuration_from` (accepts the bare configuration object OR unwraps a `{"configuration":{...}}` wrapper); the `--config` help documents the real fields. Live-verified end-to-end: `create-schedule` (Cron interval 30) → 201; `update-schedule --config` (Daily times) → updated. Pure `schedule_configuration_from` is unit-tested; offline e2e assert the body nests `configuration` (not a top-level spread).
...
- **Run on demand uses `jobType=Execute` (NOT `CopyJob`)**: `POST /workspaces/{ws}/items/{id}/jobs/instances?jobType=Execute` triggers a copy job → `202 Accepted`. The query-param job type is `Execute`; confusingly, the resulting instance's OWN `jobType` field (in `GET .../copyJobs/{id}/jobs/instances/{jobId}`) is reported as `CopyJob`. `copy-job run` uses `Execute`.
- **The 202 run response has an EMPTY body; the instance id is in the `Location` header**: `.../jobs/instances/{instanceId}`. A naive `body["id"]` extraction returns empty (this was a latent `--wait` bug shared by `data-pipeline run`). Use `client::trigger_item_job`, which reads the id from `Location` (last path segment). Poll the instance at `/workspaces/{ws}/items/{id}/jobs/instances/{instanceId}` for `status` (Completed/Failed/Cancelled/Deduped).
- **Data-movement DEFINITION authoring is portal-oriented (not cleanly REST-authorable)**: The `copyjob-content.json` `properties.source`/`destination` + `activities[]` shape is documented, but authoring a runnable Lakehouse→Lakehouse copy job over `updateDefinition` (identity-based, no external connection) is rejected server-side with a generic `UnknownError: An error occurred while processing the operation` — the `properties` block persisted but the `activities` array was silently dropped (`activities:[]`), so the run completes trivially copying nothing. The portal copy-assistant "Sample data" (NYC-Taxi) source is a portal-only connector. `copy-job run` itself is fully API-backed and validated; copy-job data-movement authoring remains portal-gated.

## Dataflow API Behaviors Discovered
- **`run` requires a non-empty `queriesMetadata` in `queryMetadata.json` (live-verified Aug 2026)**: A REST-authored Dataflow Gen2 whose `queryMetadata.json` lacks a `queriesMetadata` object fails at `dataflow run` with `Dataflow run failed: Invalid QueriesMetadata for dataflow with id X. QueriesMetadata must not be empty` — even when `mashup.pq` has valid `shared` queries. Each `shared` query needs an entry: `"queriesMetadata": {"<QueryName>": {"queryId": "<uuid>", "queryName": "<QueryName>", "loadEnabled": true|false}}`. With that added, `run` completes (`status: Completed`; `loadEnabled:false` stages the query result without a destination — live-verified). fabio's `dataflow run` now enriches this specific failure with a teaching hint showing the exact `queriesMetadata` shape (`dataflow_run_failure_hint`, unit-tested + live-verified). `discover-parameters` on a non-parametric dataflow returns a clean `DataflowIsNotParametricError`.
- **Lakehouse data DESTINATION is portal-generated, NOT cleanly REST-authorable (live-observed)**: The Dataflow Gen2 tutorial's core step — writing the transformed query to a lakehouse table — could not be reproduced by hand-authoring the destination binding. A `queriesMetadata.<Query>.dataDestinations[]` entry (workspaceId/itemId/tableName/updateMethod) + a `shared <Query>_DataDestination = Lakehouse.Contents(...)` mashup query is ACCEPTED by `updateDefinition` and the subsequent `run` reports `status: Completed`, but NO table lands in the lakehouse (`list-tables` empty). The real destination binding (the generated M `Lakehouse.Contents` write pattern + the exact queriesMetadata `dataDestination` shape + a connection) is provisioned by the portal's "Add data destination" authoring flow — mirroring the copy-job data-movement portal-orientation. So a purely REST-authored Dataflow Gen2 can transform/stage but not reliably persist to a destination; the create/update-definition/run/discover-parameters/execute-query commands all work.
...
- **`execute-query` needs a portal-registered query, not just a `shared` mashup member (platform nuance, not a fabio bug)**: `POST /workspaces/{ws}/dataflows/{id}/executeQuery` with `{queryName}` returns `400 DataflowExecuteQueryError: "Query name not found"` for a query that IS present in `mashup.pq` as a bare `section Section1; shared MyQuery = …` (verified: the mashup round-trips through get-definition, the name is exact). The executeQuery runtime resolves queries from the dataflow's internal query-registration/entity metadata (query groups, staging entities) that the Fabric **portal** authoring experience provisions — a minimal REST-authored `mashup.pq` + `queryMetadata.json` is a valid, storable definition but does NOT register a query for the execution runtime. So `execute-query` (and `--mashup` override) is reliable only against a portal-authored/edited Dataflow Gen2. fabio's command shape is correct — the error is the platform's.
- **Bytes-endpoint error surfacing was dropping the real message (fabio diagnostics fix)**: `dataflow execute-query` returned a useless `HTTP 400 Bad Request` instead of the real `Query name not found`. Root cause: the Fabric *bytes* client helpers (`post_fabric_bytes`, `post_fabric_bytes_with_accept` → `handle_bytes_lro_response`, both the immediate-error and LRO-poll-error branches) parsed ONLY the nested `{"error":{"message"}}` envelope and fell back to a bare `HTTP {status}`, discarding the body. But `executeQuery` (like OneLake/DFS) returns a TOP-LEVEL `{"errorCode":…, "message":…}` envelope, so the message was dropped. Fixed by a shared `error_message_from_body(status, text)` free fn (mirrors `handle_response`: tries `error.message`, then top-level `message`, then a truncated+redacted raw body — never a bare status) now used by all three bytes-error sites. The PowerBI bytes helpers were already fine (they pass the raw body via `FabioError::from_status`). Unit-tested (`error_message_from_body_handles_both_envelope_shapes`, incl. the dataflow regression); live-verified `dataflow execute-query` now reports `Query name not found`.
...
- **Create is LRO**: Returns 202, requires polling.
- **An ML Model item IS an MLflow registered model (verified live)**: Creating an `mlModels` item immediately registers a model in the workspace MLflow **model registry** whose `name` == the item's `displayName` and whose `artifact_id` == the item GUID (`GET /workspaces/{ws}/mlflow/api/2.0/mlflow/registered-models/get?name=<displayName>` returns it with `latest_versions:[]` for a fresh model). The registry is keyed by NAME, not GUID. **TWO distinct "version" concepts**: (1) the serving-ENDPOINT versions at `/mlModels/{id}/endpoint/versions` (`ml-model list-versions`/`get-version`), and (2) the MLflow model-registry TRAINED versions (`ml-model list-registry-versions`/`get-registry-version`). Registry reads: `GET .../mlflow/api/2.0/mlflow/model-versions/search?filter=name='<name>'` → `{model_versions:[{name, version, current_stage, status, run_id, source, creation_timestamp, …}]}`; `GET .../model-versions/get?name=<name>&version=<n>` → `{model_version:{…}}`. A model version can be seeded entirely over REST for testing: `POST .../registered-models/get` (or item create) → create a run (`POST .../runs/create` with `experiment_id`) → `POST .../model-versions/create` with `{name, source, run_id}` (the `run_id` MUST be a valid run GUID — `model-versions/create` rejects an empty/invalid run id with `BAD_REQUEST: Run Id should not be empty and needs to be a valid GUID`). New versions start `current_stage:"None"`, `status:"READY"`, and `source` is rewritten to an `abfss://…onelake…` OneLake URI. The MLflow model-registry endpoints require a **capacity-assigned** workspace (a capacity-less workspace returns `WorkspaceHasNoCapacityAssigned`, NOT `NotFound` — which is how their availability was confirmed). NOTE MLflow 2.0 `registered-models/list` is gone (returns `EntityNotFound`); use `registered-models/search`. **`get-registry-version` unwraps the `{model_version:{…}}` envelope (consistency fix)**: it previously rendered the raw MLflow wrapper (`{"data":{"model_version":{…}}}`), so `--query status` / `data.status` was null — inconsistent with `list-registry-versions` (flattened rows) and `ml-experiment get-run` (which unwraps `{run:{…}}`). Now unwraps to `{"data":{name, version, status, run_id, source, …}}`. Live-verified: `get-registry-version --version 1` → `{name:"SalesModel", version:"1", status:"READY", run_id:…}` at top level.
...
- **`variable-library update-definition --file` accepts a full multi-part envelope (fixed, reproducing the Variable Library tutorial)**: The command previously ALWAYS wrapped the file bytes as a single `variables.json` part, so it could NOT author `settings.json` + `valueSets/*.json` (alternate value sets) — pushing a full envelope produced `InvalidContent (VariablesValidationErrorCode)` because the whole envelope JSON landed inside `variables.json`. It now routes through `definition_spec::build_update_definition_body(script, "variables.json")`, accepting EITHER a full `{definition:{parts:[…]}}`/`{parts:[…]}` envelope (passed through verbatim — the way to create value sets) OR a raw `variables.json` (wrapped, backward-compatible). Same class of fix as `graphql-api update-definition`. Live-validated: authored a library with variables + `Test`/`Prod` value sets and activated one (`variable_library_update_definition_multipart_via_cli`).
...
- **Definition is ONLY a lakehouse pointer — the entity model/mappings are NOT in the item definition (live-confirmed Aug 2026)**: `get-definition` of a DTB returns exactly two parts — `definition.json` = `{"LakehouseId":"<dtdm-lh-uuid>"}` and `.platform`. The ontology model (namespaces, entity types, properties, relationship types) and the source→entity MAPPINGS live INSIDE the auto-provisioned `<name>dtdm` lakehouse (portal-managed Delta tables/views), NOT in the item definition — so there is NOTHING extra to author via `update-definition`, confirming modeling/mapping/contextualization is genuinely portal-only (no REST surface). A FRESH (unmodeled) DTB's dtdm lakehouse SQL endpoint has only the `dbo` + `sys` schemas and zero tables/views; the `dom` schema (the queryable **domain views** a modeled twin exposes) is created only after the portal modeling step. fabio's REST-addressable value is therefore the CRUD + data-access surface: `show-lakehouse` (resolves the dtdm lakehouse from `definition.json` `LakehouseId`) and `query` (T-SQL against the dtdm SQL endpoint — `dom.<Entity>` domain views once modeled) — both live-verified working; once a twin is modeled+materialized in the portal, `digital-twin-builder query --sql "SELECT … FROM dom.<Entity>"` reads it.
- **Definition file**: `definition.json` (standard path).
...
- **`create --adf-id` mounts a real Azure Data Factory (live-verified Aug 2026)**: `fabio mounted-data-factory create --workspace <ws> --name <n> --adf-id <ARM-resource-id>` builds the `mountedDataFactory-content.json` from the ADF ARM id and creates the mount — live-verified end-to-end against a real `az datafactory create` factory: `get-definition` round-trips `{"dataFactoryResourceId":"/subscriptions/…/factories/<name>"}`. No connection object is needed for the mount itself (the ARM id + the caller's Fabric identity suffice for creation).
...
### Cosmos DB data-plane (NoSQL REST) — live-validated
Fabric's Cosmos DB uses the **same engine as Azure Cosmos DB for NoSQL**, so fabio speaks the documented NoSQL REST data-plane directly (transport: `src/commands/cosmos_db_database/data_plane.rs`). All of the following were validated live against the tenant (Aug 2026):
- **Endpoint discovery**: `GET /workspaces/{ws}/cosmosDbDatabases/{id}` returns `properties.serverFqdn` (e.g. `https://<id>.z64.sql.cosmos.fabric.microsoft.com`) and `properties.databaseName`. The Cosmos **database name = the Fabric item display name**. `serverFqdn` ends in `.fabric.microsoft.com` so it passes `validate_trusted_url` with no trust-list change. A `--endpoint` override is available as a fallback.
- **Auth scope**: `https://cosmos.azure.com/.default` (new scope `COSMOS_SCOPE`; `FABIO_COSMOS_SCOPE` override, `FABIO_COSMOS_ACCESS_TOKEN` static-token env). Entra-ID only.
- **AAD authorization header (no HMAC signing)**: the Cosmos REST `authorization` header for an AAD token is `type=aad&ver=1.0&sig=<jwt>`, **URL-encoded whole** (`%3D`/`%26` for `=`/`&`). This avoids the master-key HMAC-SHA256 signing entirely. Requires `x-ms-date` (RFC1123 GMT) and `x-ms-version` (fabio pins `2018-12-31`).
- **Autoscale-only throughput**: creating a container with a manual `offerThroughput` (or no throughput) fails `400 BadRequest "Offer Type is restricted to Autoscale for your account."`. A container MUST be created with the autoscale header `x-ms-cosmos-offer-autopilot-settings: {"maxThroughput":N}` (autoscale minimum is 1000 RU/s — fabio's `--autoscale-max` default). `create-container` body: `POST /dbs/{db}/colls` `{"id","partitionKey":{"paths":["/pk"],"kind":"Hash","version":2}}` + the autopilot header.
- **List containers**: `GET /dbs/{db}/colls` → `{"DocumentCollections":[...],"_count":N}`.
- **Query**: `POST /dbs/{db}/colls/{coll}/docs` with `Content-Type: application/query+json`, `x-ms-documentdb-isquery: true`, body `{"query","parameters":[{"name":"@p","value":...}]}` → `{"Documents":[...],"_count":N}`. **Cross-partition must be explicitly enabled** (`x-ms-documentdb-query-enablecrosspartition: true`) or the query fails `400 "Cross partition query is required but disabled"` — fabio enables it automatically whenever no `--partition-key` is given; a `--partition-key` scopes via `x-ms-documentdb-partitionkey: [<jsonvalue>]`. RU cost is in the `x-ms-request-charge` response header; page continuation is in `x-ms-continuation`.
- **Upsert document**: `POST /dbs/{db}/colls/{coll}/docs` with `x-ms-documentdb-is-upsert: true` + `x-ms-documentdb-partitionkey: [<jsonvalue>]`. The partition-key header value is a JSON array whose element must be the raw JSON value (string quoted, number bare) — fabio derives it from the container's `partitionKey.paths[0]` during `import`. There is NO native bulk REST endpoint; `import` does bounded parallel upserts (`parallel::execute_parallel` + `BatchSummary`), so it is idempotent and retry-safe.
- **Partition-key value typing**: numeric/boolean keys must NOT be quoted in the header (`[42]`, not `["42"]`); a leading-zero id like `"00123"` stays a string. fabio's `cli_partition_value` only treats bare number/bool/null as typed.
- **Single-document point ops need the partition key**: `get-document` (`GET .../docs/{id}`) and `delete-document` (`DELETE .../docs/{id}`) both require `x-ms-documentdb-partitionkey: [<value>]` — Cosmos has no cross-partition point read/delete. fabio surfaces this as a required `--partition-key`. `create-document` derives the key from the document via the container's partition-key path (like `import`) unless `--partition-key` is given.
- **Export strips system metadata**: a `SELECT * FROM c` returns each document with Cosmos server-generated fields `_rid`, `_self`, `_etag`, `_attachments`, `_ts`. `export` removes them so the JSONL is clean, re-importable user data (`export --output-file x.jsonl` → `import --source x.jsonl` roundtrips cleanly, live-verified). Without `--output-file`, export streams raw JSONL to stdout (no envelope) for piping.
...
- **refresh-metadata supports selective table sync**: The `POST /workspaces/{ws}/sqlEndpoints/{id}/refreshMetadata` LRO accepts optional `timeout`, `recreateTables`, and `tables` fields. `timeout` is a Duration object (`{"value":10,"timeUnit":"Minutes"}`), not an ISO-8601 string; timeUnit is PascalCase `Seconds|Minutes|Hours|Days`. `tables` is an array of `{"schema":"...","tableNames":["..."]}` groups with at most 25 total tables; omitting it or sending an empty array refreshes all tables. `recreateTables: true` scopes the sync-state reset to the selected tables when `tables` is present. Schema and table names are 1-128 characters; schema is required and cannot be a reserved system schema. Schema-enabled parent items resolve the supplied schema and return schema-qualified `tableName` values; non-schema-enabled items resolve under the default schema, and non-default schema selections report `DeltaTableNotFound`.
- **refresh-metadata returns table sync status**: The LRO returns `{"value":[{tableName, status, startDateTime, endDateTime, lastSuccessfulSyncDateTime, error?}]}` — a per-table sync-result list. `status` is `Success` (synced this run), `NotRun` (already up to date), or `Failure`; failures can include `error.errorCode` and `error.message`. `sql-endpoint refresh-metadata` renders the `value` array as a proper list (`{data:[…], count}` + table view) so agents can filter (e.g. `--query "[?status!='Success']"`). Live-verified: a freshly loaded table shows `Success` on the first refresh and `NotRun` on an immediate second refresh.
...
- **OneLake external location for Unity Catalog — the item is a shell; the value is its OneLake path (`external-location` helper)**: The June-2026 "Azure Databricks can store Unity Catalog managed tables directly in OneLake" (Beta) feature uses this item as the Fabric-side storage anchor. The item's create body is just `{displayName, description?, folderId?, definition?, sensitivityLabelSettings?}` (the `definition.json` part is an empty `{}`) and the Get response is bare metadata (`{id, displayName, description, type, workspaceId}`) — **it carries NO OneLake path**. But Azure Databricks REQUIRES the exact **ID-based ABFSS** path `abfss://<WorkspaceID>@onelake.dfs.fabric.microsoft.com/<DatabricksStorageID>/Files/` for its UC external-location URL: name-based paths are **rejected at creation time**, and the `/Files/` suffix is **mandatory** (the docs' troubleshooting explicitly flags a missing `/Files` folder). So fabio adds `azure-databricks-storage external-location --workspace <ws> --id <id>` (read-only) which validates the item type then constructs+prints that path plus the setup prerequisites. **Live-validated against real Azure Databricks**: the fabio path was accepted by Databricks as a valid OneLake path — creating the UC external location returned `Cannot create … because the path URL belongs to Fabric OneLake` (Databricks correctly identified the path; the create is then gated by the Databricks workspace's Beta OneLake-external-location support, which needs DBR 18.1+ or serverless — a Databricks-side prerequisite). Setup gotchas confirmed live: (a) the Access-Connector Managed Identity needs an Azure **Reader** role on itself AND the caller creating the UC storage credential needs **Contributor** on the access connector; (b) the MI needs an Admin/Member/Contributor **Fabric workspace role** (`workspace add-role-assignment --principal-type ServicePrincipal`); (c) the tenant setting `AllowOneLakeUDK` ("Authenticate with OneLake user-delegated SAS tokens") must be enabled (delegated to workspace) plus `ArtifactDatabricksStoragePreview`. **Any principal with a non-Viewer role in the workspace can read/write/delete the storage item's data** (a documented security caveat — use a dedicated, restricted workspace for these items). The reverse "Publish to Fabric" direction creates a `MirroredAzureDatabricksCatalog` (covered by that group).
...
- **`numberOfMemberGateways` is required**: Must be between 1 and 11. Default in CLI: 1. Not applicable to `StreamingVirtualNetwork` gateways.
...
- **`list-scopes`/`list-tables` require a `connectionId` query param — the commands were broken (found & fixed live)**: `GET /workspaces/{ws}/catalogmirroring/scopes` and `.../tables` REQUIRE `?connectionId=<id>` (the catalog mirroring source connection) in addition to `?beta=true` (`scopes` also takes optional `?parent`/`?recursive`; `tables` takes optional `?scope`). fabio sent only `?beta=true`, so both failed with `InvalidInput: The request has an invalid input`. Fixed: both gained a required `--connection-id` (+ `--parent`/`--recursive` for scopes, `--scope` for tables). Live-verified the shape: WITHOUT `connectionId` → generic `InvalidInput`; WITH it (a probe id) → `InvalidInput: The connection '<id>' was not found or is not accessible` (a SEMANTIC response — the param is accepted and the connection resolver runs). Pure `build_scopes_url`/`build_tables_url` are unit-tested. A full happy path needs a real catalog-mirroring connection.
...
## Mirrored Google Lakehouse Runtime Catalog API Behaviors Discovered
- **New preview item type (Aug 2026)**: `MirroredGoogleLakehouseRuntimeCatalog` mirrors a Google Cloud "Lakehouse runtime catalog" (Google BigLake managed Apache Iceberg V2 tables — V1 unsupported) into OneLake. Only the catalog structure is mirrored (no data movement); the underlying Iceberg data stays in Google Cloud Storage and is read via auto-created shortcuts. Read-only in Fabric, queried via an auto-provisioned SQL analytics endpoint. Docs: `learn.microsoft.com/fabric/mirroring/catalog-mirroring/google-lakehouse-runtime[-tutorial|-limitations]`.
- **Item type string is `MirroredGoogleLakehouseRuntimeCatalog`** (authoritative — appears in the docs and in the Workload Identity Federation OIDC audience `https://analysis.windows.net/powerbi/connector/MirroredGoogleLakehouseRuntimeCatalog`).
- **Endpoint pattern (predicted, Fabric convention)**: `/workspaces/{ws}/mirroredGoogleLakehouseRuntimeCatalogs/{id}` + `getDefinition`/`updateDefinition`, `refreshCatalogMetadata?beta=true` (POST), `mirroringStatus?beta=true` (GET), `tablesMirroringStatus?beta=true` (GET). Workspace-level scope/table discovery reuses the shared `catalogmirroring/scopes|tables?beta=true&connectionId=<id>` endpoints (same as `mirrored-catalog`).
- **Definition file**: `mirroring.json` (same as Mirrored Database / Mirrored Catalog).
- **Tenant setting**: gated by `ArtifactMirroredCatalogPreview` ("Enable new mirrored catalog items (preview)") per the tutorial prerequisites — the SAME setting as `mirrored-catalog`. Verified enabled in the fabio test tenant.
- **Auth is Google Cloud Workload Identity Federation, NOT a service-account key**: Fabric presents a Microsoft Entra OIDC token to Google Cloud. Google-side config: a Workload Identity Pool + OIDC provider with Issuer `https://sts.windows.net/{TENANT_ID}/`, audience `https://analysis.windows.net/powerbi/connector/MirroredGoogleLakehouseRuntimeCatalog`, attribute mapping `google.subject = assertion.oid`; grant the federated principal `roles/biglake.viewer` + `roles/serviceusage.serviceUsageConsumer` (+ storage read via credential vending or end-user creds). Connection values: URL `https://biglake.googleapis.com/iceberg/v1/restcatalog`, Warehouse `bl://projects/{PROJECT_ID}/catalogs/{CATALOG_ID}` (multi-bucket) or `gs://{BUCKET}` (single-bucket), plus project number / pool ID / provider ID.
- **REST surface NOT yet rolled out to the fabio test tenant/region (verified live, Aug 2026)**: `GET /workspaces/{ws}/mirroredGoogleLakehouseRuntimeCatalogs` returns `NOT_FOUND` (unlike `mirroredCatalogs`/`mirroredAzureDatabricksCatalogs`, which return an empty `{value:[]}`), and `POST /workspaces/{ws}/items` with `type: MirroredGoogleLakehouseRuntimeCatalog` returns `InvalidItemType: Requested item type 'MirroredGoogleLakehouseRuntimeCatalog' is invalid` — even with `ArtifactMirroredCatalogPreview` enabled. So the typed CRUD endpoint isn't deployed here yet. fabio's command group is wired on the standard mirrored-catalog pattern (deterministic endpoint naming); dry-run + clap validation + the structured NOT_FOUND are validated live, but the typed happy path awaits Microsoft's API rollout. Re-run the live E2E once `list` returns `{value:[]}` instead of `NOT_FOUND`, and re-verify the `ArtifactMirroredCatalogPreview` gating (toggle off → expect a feature-disabled 403 on the REST path, then re-enable) per the Tenant-Feature Gates rule.
- **The CONNECTION type is rolled out AHEAD of the ITEM type (verified live, Aug 2026)**: `connection list-supported-types` DOES return a `MirroredGoogleLakehouseRuntimeCatalog` connection type in the fabio test tenant, even though the item type is still `InvalidItemType`. Its single creation method `MirroredGoogleLakehouseRuntimeCatalog.Contents` requires 5 Text params — `warehouse`, `billingProject`, `projectNumber`, `poolId`, `providerId` (exactly the tutorial's connection values: warehouse `bl://…`/`gs://…`, GCP billing project ID, numeric project number, Workload Identity Pool ID, OIDC provider ID) — with `supportedCredentialTypes: [OAuth2]`, `supportedConnectionEncryptionTypes: [NotEncrypted]`, `supportsSkipTestConnection: false`. fabio's `connection create --connection-type MirroredGoogleLakehouseRuntimeCatalog --creation-method MirroredGoogleLakehouseRuntimeCatalog.Contents --parameters '{…5 params…}' --credential-type OAuth2` builds a correctly-shaped body (verified via `-v`: `connectionDetails.type/creationMethod/parameters` all correct, `connectionEncryption: NotEncrypted`). A live create returns generic `InvalidInput` because the OAuth2 credential is the Microsoft-Entra→Google-WIF federated token, which is minted by an INTERACTIVE consent flow (browser) — it can't be supplied non-interactively without a real Google Cloud Workload Identity Pool/provider that trusts our tenant. `supportsSkipTestConnection: false` means the API always test-connects, so a dummy connection can't be created regardless.
- **Can the feature be enabled / e2e-tested in the fabio tenant? NO (verified live, Aug 2026)**: (1) There is NOTHING left to toggle — the documented gate `ArtifactMirroredCatalogPreview` is already ON; the only other Google-named settings are `GoogleBigQuerySSO` (a BigQuery-connection SSO toggle, unrelated) and `OneLakeCatalogSubItemDiscovery` (search). (2) The blocker is a REGION/RING rollout gap, not a setting: the item type is unregistered in the tenant's Fabric backend (`InvalidItemType`) across all our capacities (West Europe / Sweden Central / Central US), so the mirrored item cannot be created via REST or portal. (3) A real end-to-end ALSO needs external Google Cloud infrastructure we don't have — a GCP project with billing + the BigLake API, a Lakehouse runtime catalog with Apache Iceberg V2 tables, and a Workload Identity Pool + OIDC provider configured to trust our Entra tenant — plus the interactive OAuth2 consent. Conclusion: a happy-path e2e is impossible until BOTH the item type rolls out to our region AND a GCP BigLake source is provisioned. The offline/validation e2e suite (dry-run, destructive signal, clap validation, tolerant list-or-NOT_FOUND) passes live today; the item-type happy-path test stays `#[ignore]` pending rollout.



## Mirrored Databricks Catalog API Behaviors Discovered
- **A freshly created mirror never syncs until `autoSync` is Enabled — `update --auto-sync` is the missing activator (gap fixed, live-validated)**: After `create` with a valid creationPayload, the mirror sits at `autoSync: Disabled` / `mirrorStatus: NotMirrored` / `syncDetails.lastSyncDateTime: null` and NEVER replicates. The `creationPayload` has NO `autoSync` field (confirmed in the swagger — only `catalogName`/`databricksWorkspaceConnectionId`/`mirroringMode`/`storageConnectionId`), so activation is impossible at create time. The activator is the `MirroredAzureDatabricksCatalogUpdatePayload` (`{autoSync, mirroringMode, storageConnectionId}`) sent via `PATCH /workspaces/{ws}/mirroredAzureDatabricksCatalogs/{id}` under a `properties` object. fabio's `update` previously only handled name/description; added `--auto-sync Enabled|Disabled [--mirroring-mode] [--storage-connection-id]`, nested under `properties`. **The PATCH REQUIRES a top-level `displayName` whenever `properties` is present** — a properties-only body is rejected with `UnknownError: Invalid Display Name: ''`, so `update` GETs the current display name and includes it when `--name` is not supplied. Live-verified: `update --auto-sync Enabled` flipped the mirror to `mirrorStatus: Mirrored` / `lastSync: <ts>` / `errorInfo: None` and the Databricks schema replicated to OneLake (`Tables/default/` appeared). Pure `build_update_body` (nests `autoSync`/`mirroringMode`/`storageConnectionId` under `properties`, keeps `displayName`) is unit-tested; offline dry-run e2e + live-validated.
- **`warehouse query` now resolves a mirror's read-only SQL endpoint**: A `MirroredAzureDatabricksCatalog` exposes a SQL analytics endpoint at `properties.sqlEndpointProperties.connectionString` (same shape as a lakehouse). `warehouse/get_connection_string` only tried warehouse/lakehouse/warehouseSnapshot, so `warehouse query --id <mirror>` failed with `Could not determine SQL connection string`. Added the mirror fallback (GET `/mirroredAzureDatabricksCatalogs/{id}` → `sqlEndpointProperties.connectionString`), same pattern as the earlier WarehouseSnapshot fix. Live-verified: system tables + `INFORMATION_SCHEMA.TABLES` are queryable through the mirror endpoint.
- **A Databricks MANAGED table in Databricks-managed storage is NOT queryable via the mirror SQL endpoint — deletion vectors are a red herring (Databricks-Fabric interop limitation, NOT a fabio bug)**: The SQL-endpoint metadata sync for a mirrored table (`sql-endpoint refresh-metadata` → per-table `Failure`) returns a storage `BadRequest 400` (`x-ms-remote-data-destination-type`). It was FIRST hypothesized to be deletion vectors (the table shipped with the modern defaults `delta.enableDeletionVectors=true` + `minReaderVersion=3`). **Live-disproven end-to-end**: recreating the table with `TBLPROPERTIES ('delta.enableDeletionVectors' = false)` → `minReaderVersion=1` DID make the mirror re-sync cleanly (`mirror show`: `mirrorStatus: Mirrored`, a fresh `lastSyncDateTime`, `errorInfo: None`) — proving the DV fix works at the MIRROR level — yet `sql-endpoint refresh-metadata` STILL returns the same `BadRequest`. The REAL gate is the table's STORAGE: `DESCRIBE EXTENDED` shows `Type: MANAGED`, `Location: abfss://unity-catalog-storage@<dbstorage>.dfs.core.windows.net/…` (the Databricks-managed metastore storage account, in the `databricks-mrg-*` managed resource group). Fabric's SQL endpoint reads the mirrored table via a OneLake shortcut to that ADLS and has no credentials for it, and you cannot grant them (the managed RG has an Azure deny-assignment). So DV=false is NECESSARY (for the mirror re-sync) but NOT SUFFICIENT (for the SQL-endpoint read). To make a Databricks table queryable through the mirror it must NOT be a MANAGED table in Databricks-managed storage: (a) an **EXTERNAL table in a customer-controlled ADLS** + a Fabric ADLS connection set via `mirrored-databricks-catalog update --storage-connection-id`, or (b) a table STORED IN OneLake via the "Azure Databricks Storage" external-location flow (read natively, no credential vending). The fabio surface (`update --auto-sync` activation + `warehouse query` endpoint resolution) is complete; the remaining constraint is the platform's. NOTE the mirror re-replicates a recreated table on the autoSync interval (not instantly) — `lastSyncDateTime` must advance past the recreation first.
- **`mirrored-databricks-catalog update --storage-connection-id` is IGNORED by the per-table `DatabricksCatalog` shortcut — workaround (a) does NOT resolve the SQL-endpoint read (live-disproven end-to-end)**: Pursued the "EXTERNAL table in customer-controlled ADLS + `--storage-connection-id`" workaround fully and live: created an ADLS Gen2 account, granted the Databricks Access-Connector MI `Storage Blob Data Contributor`, created a Databricks external location on it, recreated `sales` as an EXTERNAL Delta table there (DV off), created a Fabric `AzureDataLakeStorage` ServicePrincipal connection (the mirror REJECTS a `Key`-credential storage connection: "Storage CredentialType 'Key' is not supported. Supported: OAuth2, ServicePrincipal, WorkspaceIdentity"), granted the SP `Storage Blob Data Reader` (verified the SP can read the ADLS directly), and set it as the mirror's `storageConnectionId` (confirmed on `show`). The mirror re-synced and the OneLake shortcut re-pointed to the customer ADLS (`abfss://ext@<sa>.dfs.core.windows.net/ext/sales…`). **But the SQL endpoint STILL returns the same `BadRequest 400` (`x-ms-remote-data-destination-type`)** — because the per-table shortcut is of type `DatabricksCatalog` and ALWAYS carries `connectionId = <the Databricks PAT connection>`, NOT the `storageConnectionId`. Even a FRESHLY-created shortcut (a brand-new external table created AFTER the storageConnectionId was set) still referenced the Databricks connection. So Fabric reads a mirrored Databricks table exclusively via **Databricks credential vending through the PAT connection**, and the `storageConnectionId` is not applied to `DatabricksCatalog` shortcuts — the credential-vending read fails with `BadRequest` in this setup regardless. Net: BOTH documented workarounds are blocked here — (a) storageConnectionId is ignored + PAT credential-vending fails, (b) OneLake external location needs the Databricks Beta (DBR 18.1+/serverless). This is a Databricks-Fabric platform limitation, NOT a fabio gap: every fabio command (create, `update --auto-sync`, `update --storage-connection-id`, `warehouse query` endpoint resolution, `sql-endpoint refresh-metadata`) works correctly and faithfully surfaces the platform error.
- **`create` couldn't actually mirror — no `creationPayload` flags (gap fixed, validated against a REAL Databricks workspace)**: `mirrored-databricks-catalog create` only sent `{displayName}`, so it created an EMPTY SHELL — `MirroredAzureDatabricksCatalogCreationPayload` requires `catalogName` + `databricksWorkspaceConnectionId` + `mirroringMode` (`Full`/`Partial`) to actually mirror a Unity Catalog, and fabio had no flags for them (same class as the snowflake-database gap). Added `--databricks-connection-id` + `--catalog-name` + `--mirroring-mode` (required together via clap `requires`) + optional `--storage-connection-id`, building `body.creationPayload`. **Live-validated end-to-end against a freshly-deployed Azure Databricks Premium workspace**: deployed the workspace (ARM), minted a Databricks PAT, created a Fabric `AzureDatabricksWorkspace` connection (Key credential = PAT, `url` = `https://adb-….azuredatabricks.net`), created a Unity Catalog table (`fabio_dbx_77986.default.sales`) via the Databricks Statement Execution API, then: `discover-catalogs` → `fabio_dbx_77986` (MANAGED_CATALOG), `discover-schemas --catalog-name` → `default`, `discover-tables --schema-name default` → `sales` (all three confirm the `--connection-id` fix against real Databricks), and `create --databricks-connection-id <conn> --catalog-name fabio_dbx_77986 --mirroring-mode Full` → a REAL mirror (`show` → `catalogName:fabio_dbx_77986, mirroringMode:Full, syncDetails.status:Success`, SQL endpoint provisioned). `mirrorStatus` stays `NotMirrored` until `autoSync` is Enabled (see the activation note above). The `discover-*` responses are a `{value:[…]}` envelope (Databricks Unity Catalog shapes: `{name, fullName, catalogType, storageLocation}`).
- **`discover-catalogs`/`discover-schemas`/`discover-tables` require a `databricksWorkspaceConnectionId` query param — the commands were broken (found & fixed live)**: `GET /workspaces/{ws}/azureDatabricks/catalogs[/{cat}/schemas[/{schema}/tables]]` REQUIRE `?databricksWorkspaceConnectionId=<id>` (the Databricks workspace connection). fabio sent none, so all three failed. Fixed: each gained a required `--connection-id`. Live-verified the shape: WITH a probe id → `BadRequest: PowerBINotAuthorizedException` (the request reached the Databricks discovery backend — the param is accepted), vs the old no-param 400. A full happy path needs a real Databricks workspace connection.
...
- **A WarehouseSnapshot is T-SQL-queryable via `warehouse query` (gap fixed)**: A snapshot's `show` carries `properties.connectionString` (its OWN read-only SQL endpoint, same shape as a warehouse — e.g. `xxx.datawarehouse.fabric.microsoft.com`), so `fabio warehouse query --id <snapshotId> --sql …` should work. But `warehouse::get_connection_string` only tried `/warehouses/{id}` then `/lakehouses/{id}` and errored (`Could not determine SQL connection string`) for a WarehouseSnapshot id. Added a third fallback: `/workspaces/{ws}/warehouseSnapshots/{id}` → `properties.connectionString` (database name = the snapshot's `displayName`). Live-verified: a snapshot taken AFTER 3 rows returns all 3 rows with data, while the parent warehouse (now modified) and the snapshot diverge — proving both the query path AND point-in-time isolation. NOTE the snapshot's point-in-time is exact: a snapshot created with an explicit `--snapshot-datetime` captured the instant before recent writes were durable at that timestamp returned 0 rows (Fabric point-in-time semantics governed by the warehouse time-travel retention window, not a fabio issue); a "now" snapshot (omit `--snapshot-datetime`) sees the current committed state. Live e2e `warehouse_snapshot_is_tsql_queryable` (gated on `FABIO_TEST_WAREHOUSE_SNAPSHOT_ID`).
...
- **`catalog search` never actually searched — wrong request field names (bug fixed)**: `CatalogQueryRequest` fields are `search` / `pageSize` / `filter` / `continuationToken`, but fabio's `build_search_body` sent `searchString` / `top` / `itemTypes` (+`excludeItemTypes`) — the API SILENTLY IGNORES unknown fields and returns a DEFAULT unfiltered listing (the first ~50 catalog entries), so `--search`, `--type`, `--top`, and `--exclude-type` all did NOTHING. Live-proven: `{searchString:"semantic", top:3}` → 50 unfiltered results (fabio_e2e_lh, …), while the correct `{search:"semantic", pageSize:3}` → 2 results that actually match "semantic". Fixed the builder to emit `search`/`pageSize`/`filter`. The `filter` is an OData-style string over the `Type` property (capital T), e.g. `Type eq 'Report' or Type eq 'Lakehouse'` (from `--type`), combined with `Type ne 'X'` clauses (from `--exclude-type`) via `and`; a lowercase `type`/`itemType` property is rejected (`Invalid filter: Property is not allowed`). fabio also now flattens the `{value:[...]}` response to the standard `{data:[...],count:N}` list envelope. Live-verified: `catalog search --search semantic --top 3` → count 2 real matches; `--type Lakehouse --top 4` → 4 Lakehouses. Pure `build_type_filter`/`build_search_body` unit-tested.
- **`catalog search` entry shape + WORKSPACE column + BODY pagination (completed)**: A search hit is an `ItemCatalogEntry` = `{id, displayName, description, catalogEntryType:"FabricItem", type:"<ItemType>", hierarchy:{workspace:{id, displayName}}}` — the owning workspace is NESTED under `hierarchy.workspace`, NOT a top-level `workspaceId` (fabio's table `WORKSPACE` column projects `hierarchy.workspace.displayName`). **Pagination lives in the BODY, not a query param**: the response carries `continuationToken`, and the next page must be requested with a body of ONLY `{continuationToken}` — repeating `search`/`filter`/`pageSize` alongside it fails with `ConflictingFilterParameters` (the token ENCODES the original search/filter, e.g. base64 `{"Skip":2,"Filter":"Type eq 'W…'"}`). Two more quirks handled: on the LAST page the API returns `continuationToken: ""` (an EMPTY STRING, not null/absent) — treat empty as "no more pages" (else the next request sends an empty token → `InvalidContinuationToken`); and a page that consumes ALL remaining results emits no token. fabio now supports `--all` (auto-paginate: post `{continuationToken}` until the token is empty/absent, accumulating pages) and the global `--continuation-token` to resume a specific page (sends ONLY the token). `/catalog/search` is the ONLY catalog operation, so `search` is the complete command surface.
- **Single command**: `search` is the only subcommand.
- **Convenience flags or raw body**: `fabio catalog search --search "..." --type Lakehouse --top N` (or `--content '{"search":"...","pageSize":N,"filter":"Type eq ..."}'` for full control).
...
- **Tag creation body format**: `POST /admin/tags/bulkCreateTags` requires `{"createTagsRequest": [{"displayName": "..."}]}`. Optional `"scope"` field: `{"type": "Tenant"}` or `{"type": "Domain", "domainId": "<uuid>"}`. Response: `{"tags": [{"id": "...", "displayName": "...", "scope": {...}}]}`. **The field name is `createTagsRequest` (an array) — NOT `createTagRequests`** (a natural-looking mistake that yields `InvalidInput: The request has an invalid input`). fabio's `admin create-tags` now offers a repeatable convenience flag `--name <displayName>` (build the body for you) alongside the raw `--file`/`--content`; the pure `build_create_tags_body` is unit-tested (asserts the `createTagsRequest` field, not `createTagRequests`). Live-verified `admin create-tags --name A --name B` creates both.
...
- **`org-app-audience create` REQUIRES `parentAppId` inside a `definition.json` part (bug fixed, live-verified Aug 2026)**: An OrgAppAudience belongs to an OrgApp, and its create is rejected with `InvalidParentAppId: The ParentAppId is missing or empty` unless the parent org-app id is supplied — but ONLY inside a `definition.json` definition part; a TOP-LEVEL `parentAppId` in the create body is ALSO rejected (verified: top-level → `InvalidParentAppId`, definition part → accepted). The `CreateOrgAppAudienceRequest` swagger schema only marks `displayName` required and its examples ship dummy base64, so the requirement is invisible from the spec. fabio's `org-app-audience create` previously sent only `{displayName, description}` (no way to pass the parent), so it could NEVER succeed. Fixed: added a REQUIRED `--org-app-id` (alias `--parent-app-id`) flag; `build_create_body` now emits a `definition.json` part = `{"$schema":".../orgAppAudienceDefinition/1.0.0/schema.json","settings":null,"parentAppId":"<org-app-id>"}` (the exact shape confirmed via `get-definition` of a live-created audience). Unit-tested (parentAppId is in the decoded definition part, NOT top-level) + live-verified end-to-end.
...
- **Virtual-network gateway member counts allow 1-11**: `numberOfMemberGateways`, `maxMemberGatewayCount`, and `minMemberGatewayCount` were expanded from 1-9 to 1-11 for create, update, and response schemas. Fabio validates the range client-side and also rejects `max < min`.
...
### PBIR report authoring — page/visual read-modify-write + scaffold (live-confirmed)

Reports have no XMLA/TOM surface; fabio authors them the same way it authors semantic models — a definition read-modify-write over the item API (`getDefinition` → edit the PBIR parts → `updateDefinition`), plus a from-scratch scaffold. Ground-truthed against a real 2-page portal-authored PBIR report. Module: `src/commands/report_def.rs` (wired into `report.rs` via `#[path]`, alias `authoring`); the definition body shape is identical to semantic models (`{definition:{parts:[{path,payload,payloadType:"InlineBase64"}]}}`).

- **Exact PBIR shapes (from a live export)**: `definition/version.json` = `{$schema versionMetadata/1.0.0, version:"2.0.0"}` (the version field is dotted-numeric, NOT the `4.0` from definition.pbir). `definition/pages/pages.json` = `{$schema pagesMetadata/1.1.0, pageOrder:[...], activePageName}`. `page.json` = `{$schema page/2.1.0, name (== folder), displayName, displayOption:"FitToPage", height:720, width:1280, objects?}` — small and scaffoldable. `visual.json` = `{$schema visualContainer/2.11.0, name (== folder), position:{x,y,z,height,width}, visual:{visualType, query?, objects?, visualContainerObjects?, drillFilterOtherVisuals}}`. A textbox's content is `visual.objects.general[0].properties.paragraphs[].textRuns[].value`; a data-bound visual's binding is `visual.query.queryState.<role>.projections[].{field, queryRef}`, where `field` is a `Column` (`{Column:{Expression:{SourceRef:{Entity}},Property}}`, queryRef `Table.Column`), an `Aggregation` (`{Aggregation:{Expression:{Column:…},Function:N}}`, queryRef `Sum(Table.Column)`; Function 0=Sum,1=Avg,2=Min,3=Max,4=Count,5=CountNonNull), or a `Measure` (`{Measure:{Expression:{SourceRef:{Entity}},Property}}`, queryRef `Table.Name`). Roles by visual type: card→`Values`; bar/column/line/pie→`Category`+`Y`; table/slicer→a single `Values` well; textbox→no query. A visual title is `visual.visualContainerObjects.title[0].properties.text.expr.Literal.Value` = `'<title>'`.
- **`list-pages` / `list-visuals`** — read-only introspection over `getDefinition` (`{name,displayName,visualCount,active}` / `{page,name,visualType,title,x/y/w/h}`); the agent-native way to understand a report before editing.
- **`add-page` / `delete-page` / `rename-page` / `set-active-page`** — RMW page authoring. `add-page` creates `definition/pages/<name>/page.json` (name defaults to a generated 20-hex, like Fabric) and registers it in `pages.json` (pageOrder + optional activePageName); `delete-page` drops the folder + deregisters it, repointing the active page, and refuses to delete the LAST page (`INVALID_INPUT`). A PBIR-Legacy report (single `report.json`, detected by the absence of `definition/pages/`) is rejected with a convert-first hint.
- **`add-visual` / `delete-visual`** — build a schema-conformant `visual.json` from a compact CLI spec: `--type <visualType> [--title] [--text] [--category Table.Column] [--measure <field>…]`. Field parsing (`parse_field_spec`) accepts `Table.Column`, `Sum/Avg/Min/Max/Count/CountNonNull(Table.Column)`, or `Measure(Table.Name)`; a bare `Table.Column` in a measure role auto-Sums. z-order = existing visuals + 1. **LIVE-PROVEN to render**: a report built with a textbox + a Sum card + a category/measure bar chart EXPORTED to a valid PDF (the export flow renders every visual — a malformed query/structure fails it).
- **`report scaffold --name --spec <json> --dataset <id> [--out <folder>] [--description]`** — generate a COMPLETE PBIR report (`definition.pbir` byConnection binding + `version.json` + `report.json` + a bundled base theme resource + one folder per page + one `visual.json` per visual + `pages.json`) from a compact spec `{"pages":[{"displayName","name"?,"active"?,"visuals":[{"type",...}]}]}`. `--out` writes the folder (validated cleanly by `report validate`); without it, creates the report via `POST /reports`. **The from-scratch `report.json` MUST carry `layoutOptimization:"None"` AND a resolvable `themeCollection`** — the API rejects a report.json missing either (`Required properties are missing … layoutOptimization, themeCollection`), and a `themeCollection.baseTheme` needs `name`+`reportVersionAtImport`+`type:"SharedResources"` PLUS a matching `resourcePackages` entry AND the actual theme JSON file — so scaffold bundles a minimal `StaticResources/SharedResources/BaseThemes/fabioTheme.json`. Live-verified: a 2-page/4-visual scaffold validates offline and, created, exports to PDF. **Creating a report with a placeholder/nonexistent `semanticmodelid` fails** (`PowerBIEntityNotFound`) — the byConnection must reference a real model.
- All mutations overwrite the definition → `mutates:true`/`destructive:true`, `--dry-run` guarded; unknown page/visual → `NOT_FOUND`, duplicate → `CONFLICT`. Pure helpers (`build_page_json`, `update_pages_json`, `parse_field_spec`, `build_visual_json`, `scaffold_parts`, plus the plumbing) are unit-tested (18 tests); the page/visual/scaffold lifecycles are live-validated (`report_page_authoring_lifecycle`, `report_visual_authoring_lifecycle`, `report_scaffold_lifecycle`) including PDF-export render proofs. Verified every edit round-trips and re-validates cleanly (pages.json integrity preserved).

### Deepened offline `report validate` (cross-reference / naming / size)

`report validate` was extended beyond structural + `$schema`-presence checks to catch broken/silently-ignored reports (all offline, no runtime deps): cross-reference integrity (`ACTIVE_PAGE_NOT_FOUND`, `DANGLING_PAGE_ORDER`, `PAGE_NOT_IN_ORDER`, `DEFAULT_PAGE_NOT_FOUND`, `DANGLING_BOOKMARK`), naming convention (`INVALID_OBJECT_NAME` — a page/visual/bookmark folder name must be word chars or hyphens else Desktop ignores it; `PAGE_NAME_MISMATCH`/`VISUAL_NAME_MISMATCH`; `DUPLICATE_PAGE_NAME`/`DUPLICATE_VISUAL_NAME` errors), service size limits (`TOO_MANY_PAGES`/`TOO_MANY_VISUALS` — 1000 each), version-format (`VERSION_FORMAT`), and byConnection deploy-readiness (`BYCONNECTION_MISSING_MODEL_ID` — the connectionString should carry `semanticmodelid=`). Validated against the real 2-page portal report (57 checks, zero findings — a well-formed report stays clean).
...
- **Predicate contract clarified by spec commit `8baaa4c`** — all three SQL
  audit surfaces return `predicateExpression` from GET and accept it on PATCH.
  Pass only the predicate expression, never the `WHERE` keyword. The maximum is
  3,000 characters. On first enable, omission means no predicate; on later
  updates omission preserves the current predicate, while an empty string
  removes it. Audit-record fields are accepted except `file_name`,
  `audit_file_offset`, and `event_time`; `action_id` and `class_type` are
  string-valued in records but predicates may compare them only to numeric
  values (no implicit string conversion). fabio validates the length and
  leading `WHERE` locally; the service validates field and operand semantics.
- **SQL endpoint refresh timeout cap** — `sql-endpoint refresh-metadata
  --timeout` still defaults server-side to 15 minutes, but the documented
  maximum is 24 hours. fabio rejects longer equivalent durations across
  Seconds/Minutes/Hours/Days before making a request.
...
### Deploy definition normalization (content-hash convergence)
- **The Fabric definition APIs NORMALIZE hand-authored content on first ingest, so `source_hash != deployed_hash` after the first deploy** (discovered building `deploy apply --verify`). A hand-written `notebook-content.py` deployed via `updateDefinition`/create is re-serialized by Fabric (metadata reordering, canonical formatting, notebook-settings auto-binding), so an immediate `deploy plan` (or `apply --verify`) re-fetches a definition whose hash differs from the local source — the item shows `update`/`converged:false` even though the deploy "succeeded". This is NOT a fabio bug: `deploy plan`'s Skip decision compares `source_item.content_hash` to `fetch_definition_hash(deployed)` (`hash_api_parts`), and `--verify` uses the SAME comparison — verified live that both agree (notebook flagged by `--verify` also showed `update` on a fresh `plan`). **Convergence workflow:** deploy once, then `deploy export` the workspace (canonical form the API returns) and deploy THAT — it converges to all-Skip / `no_changes` (verified live). `.platform` is excluded from the hash (the API rewrites `logicalId`), so metadata-only diffs never surface. Platform-only types (Lakehouse/Warehouse) carry a trivial/empty definition whose hash can also differ post-apply; `--verify` reports these as discrepancies too (consistent with `plan`), which is why data stores are best treated as existence-verified. Warehouse `updateDefinition` is `OperationNotSupportedForItem` (warehouses are not definition-updatable), so a plan that marks a Warehouse `update` will fail on apply — expected.

### Data Agent runtime consumption: MCP endpoint (Assistants API retired)
- **The OpenAI Assistants API that backed `data-agent query`/`evaluate` was retired by OpenAI on 2026-08-26**; runtime consumption of a published data agent now goes through its **Model Context Protocol (MCP) Streamable-HTTP endpoint**: `{fabricBase}/mcp/workspaces/{ws}/dataagents/{id}/agent` (the URL `data-agent mcp-url` already printed). Authoritative source: the Fabric "Fabric data agent Python SDK" doc + "Data agent as a Model Context Protocol server". Key protocol facts (verified against the official SDK sample): (1) auth is the SAME Fabric bearer token (`https://api.fabric.microsoft.com/.default`); (2) the agent exposes **exactly ONE tool**; (3) the client flow is `initialize` → `notifications/initialized` → `tools/list` (take `tools[0]`) → `tools/call` with the question bound to the tool's **first `inputSchema` property** (`next(iter(properties))`); (4) the answer is the concatenation of the result's `content[]` **text** blocks. fabio's client (`src/commands/dataagent/mcp.rs`) implements this over Streamable HTTP: it POSTs JSON-RPC with `Accept: application/json, text/event-stream`, parses BOTH a plain-JSON body and an SSE (`text/event-stream`) body (matching the response by JSON-RPC `id`), captures/echoes the `Mcp-Session-Id` header, and advertises `MCP-Protocol-Version: 2025-06-18`. Tool discovery is dynamic (no hardcoded tool name); `first_input_property` prefers a conventionally-named arg (question/query/prompt/input/text), then the first `required`, then the first declared property (robust with `preserve_order`).
- **Capabilities that were Assistants-API artifacts are GONE (not portable to MCP):** per-step SQL/DAX/KQL run-step introspection (`--show-steps`), multi-turn threads (`--thread-id`/`--keep-thread`/`threadId`), answer-attached file download (`--download-files`/`/files` API), and `VisualizeDataset` chart-spec extraction (`--visuals`). The MCP consumption surface returns the answer text only. `data-agent query` now exposes just `--prompt`/`--published-url`/`--raw`/`--stage`/`--timeout` (`--raw` surfaces the full MCP tool result); `evaluate` dropped `--show-steps`. Query output is `{question, answer, tool, raw?}` (no `threadId`/`steps`/`visuals`/`files`).

### Data Agent MCP consumption — live validation findings (Aug 2026)
