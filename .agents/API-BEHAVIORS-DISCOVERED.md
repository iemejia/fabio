...
- **`creationMethod` frequently differs from the connection `type` (verified live, July 2026)**: `connectionDetails.creationMethod` is NOT always equal to `connectionDetails.type`. `GET /connections/supportedConnectionTypes?showAllCreationMethods=true` (raw `data.value[].creationMethods[].name`) is the source of truth. Verified live: `EventHub` → `EventHub.Contents` (params `endpoint`+`entityPath`, creds include `WorkspaceIdentity`); `ConfluentCloud` → `ConfluentCloud.Contents`; `MQTT` → `MQTT.Contents`; `Oracle` → `Oracle` (matches). Sending the wrong creation method (e.g. `creationMethod: "EventHub"`) fails with `InvalidConnectionDetails: The ConnectionDetails input provided is not valid`; sending the correct `EventHub.Contents` is accepted and the API proceeds to test the credential (e.g. a `WorkspaceIdentity` against the endpoint → `DMTS_UntrustedEndpointForWorkspaceIdentity` for an unreachable/fake endpoint). fabio previously HARDCODED `creationMethod = connection_type`, which silently blocked EventHub/ConfluentCloud/MQTT (and any other type whose method differs) — including the July 2026 "Azure Event Hubs source with Workspace Identity authentication" Eventstream feature (the source references such a connection). Fixed via `connection create --creation-method <METHOD>` (optional; defaults to the connection type for backward compatibility). The connection is the auth carrier for Eventstream external sources; the eventstream side just references it.
- **Azure Event Hubs Workspace Identity connection (July 2026)**: `connection create --connection-type EventHub --creation-method EventHub.Contents --credential-type WorkspaceIdentity --parameters '{"endpoint":"sb://<ns>.servicebus.windows.net","entityPath":"<hub>"}'`. The workspace identity's service principal needs an Event Hubs data role (e.g. Azure Event Hubs Data Receiver) on the namespace. This is the connection an Eventstream `AzureEventHub` extended-features source uses for keyless (Workspace Identity) authentication.
