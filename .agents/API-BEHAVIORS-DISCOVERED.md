...
- **⚠️ Create/updateDefinition definition format CHANGED (live-confirmed, pre-existing bug)**: The Fabric API no longer accepts `definition.format: "PaginatedReportDefinition"` — it returns `InvalidDefinitionFormat`. The current expectation (probed live): OMIT the `format` field entirely, and the `.rdl` part path must be `<displayName>.rdl` (a mismatched name returns `MissingDefinitionParts: Definition for '<name>.rdl' is missing`). Even with the corrected structure and a valid RDL 2016 (ReportSections) payload, create still returns `UnknownError` — paginated-report creation via the REST definition API has additional server-side requirements not yet reverse-engineered. `fabio paginated-report create` still sends the old `format` field and is therefore currently broken; the `paginated_report_create_show_delete_lifecycle` E2E test fails for this reason. **Follow-up**: fix `paginated-report create`/`update-definition` (drop `format`, name part `<displayName>.rdl`, determine the remaining requirement). This is independent of the working `export` action.
- **Deep-dive investigation of the create blocker (all live-probed against the tenant)**: Multiple combinations were tested to pin down the requirement:
  - `format:"PaginatedReportDefinition"` + any parts → **`InvalidDefinitionFormat`** (the tenant rejects the `format` enum despite the official docs example including it — likely the format+parts validation fails together when the RDL is not renderable, surfacing as a format error).
  - No `format` + part path `report.rdl` (≠ displayName) → **`MissingDefinitionParts: Definition for '<displayName>.rdl' is missing`** — proves the part MUST be named `<displayName>.rdl` (fabio currently uses the *file basename* — this is a confirmed bug).
  - No `format` + part `<displayName>.rdl` + a hand-authored RDL (tried: RDL 2016 `Body/Page`; RDL 2016 `ReportSections`; with/without `.platform` part `{"metadata":{"type":"PaginatedReport"}}`; with proper root namespaces `MustUnderstand="df"`, `df:DefaultFontFamily`, `rd:ReportID` GUID, correct child ordering per the RDL spec) → **always `UnknownError: An unexpected error occurred while processing the request`**. The RDL renderer rejects every hand-authored payload with no diagnostics.
  - **Conclusion**: A Fabric-accepted `.rdl` almost certainly must be generated by Power BI Report Builder (element order is load-bearing and enforced by the processor, not the XSD — see the RDL format reference). Programmatic creation of a *renderable* paginated report from a hand-written RDL was not achievable in this environment. Fixing `paginated-report create` safely requires a Report-Builder-generated RDL fixture to validate against; do not change the create body blind. The `export` action does NOT depend on create and is validated independently (dry-run + live `PowerBIEntityNotFound` plumbing).
...
- **Preview status**: No `?preview=true`/`?beta=true` query flag observed in the spec's example requests for these endpoints (unlike some other preview features) — the preview designation is documentation-only in the spec at this time. fabio's `git relation` command doc comments note "(Preview)" for agent visibility, but no query-parameter gating was added; if Fabric later requires a preview flag, add it to `git_relation.rs`'s URL construction.
- **fabio implementation**: `fabio git relation list|create|delete` in `src/commands/git_relation.rs`, wired into `git.rs` as a `Relation(RelationCommand)` subcommand via `#[path = "git_relation.rs"] mod relation;` (a sibling-file submodule, not a new directory — see Key Decisions for why).
