...
- **add-datasource server response is PascalCase; fabio normalizes it (live-confirmed)**: The staging `POST .../datasources` response object uses PascalCase keys (`FabricItemType`, `Id`, `DisplayName`, `ItemReference`) — unlike the camelCase `fabricItemType` used in the REQUEST body, the `list-datasources` response, and fabio's empty-LRO fallback. Because the shape otherwise depended on LRO timing (empty 202 → fabio's camelCase fallback; populated body → server's PascalCase), fabio now recursively lower-cases the first letter of every key (`camel_case_keys`) so the output contract is stable: agents always read `data.fabricItemType`.
 ...
 - **`data-agent evaluate` is a batch primitive, not a judge**: Runs a questions file (JSON array of strings or `{question,expected}` objects, or CSV/TSV with a `question` column and optional `expected`) against the published agent — each question on its own throwaway thread, `--repeats N` times — and emits the answers (`--show-steps` adds run steps). It deliberately does NOT perform semantic/LLM grading (unlike the SDK's `evaluate_data_agent` critic); when a question carries `expected`, it adds only a NAIVE case/whitespace-insensitive `match.exact`/`match.contains` signal, leaving real judgment to the calling agent. Fails only if EVERY run errors; otherwise returns partial results with `failedRuns > 0`.
...
- **Export to file (Power BI API, not Fabric REST) — live-confirmed**: Rendering a report or paginated report to a file uses the Power BI `exportToFile` flow, NOT the Fabric item API: `POST {powerbiBase}/groups/{ws}/reports/{id}/ExportTo` (202 + job `id`) → poll `GET .../reports/{id}/exports/{jobId}` until `status: Succeeded|Failed` → `GET .../reports/{id}/exports/{jobId}/file` (binary). `fabio report export` / `paginated-report export` implement this. Format enum: PDF/PPTX (both), PNG (Power BI reports only), IMAGE/XLSX/DOCX/CSV/XML/MHTML/ACCESSIBLEPDF (paginated only). Paginated parameters go in `paginatedReportConfiguration.parameterValues`. A bogus report id returns a clean `PowerBIEntityNotFound` (404), confirming the request path/auth.
 - **⚠️ Create is blocked server-side on this tenant/capacity (definitive, live-confirmed)**: `POST /workspaces/{ws}/paginatedReports` with a definition fails on the test tenant (Sweden Central capacity) regardless of payload: **with** `definition.format:"PaginatedReportDefinition"` → `InvalidDefinitionFormat`; **without** the `format` field → `UnknownError`. This was proven with a **real, security-reviewed Report Builder 2016 RDL** (the `enter-data-starter.rdl` inline-data template: no external data source, no embedded VB `<Code>`, no external images — only `ENTERDATA` inline rows + `=Fields!…`/`=Globals!PageNumber` expressions), so **the RDL content is NOT the problem** — an earlier hypothesis blamed hand-authored RDLs, but a valid one fails identically. The part path must be `<displayName>.rdl` (a mismatch returns `MissingDefinitionParts: Definition for '<displayName>.rdl' is missing`). Because the official docs example uses `format:"PaginatedReportDefinition"` (which `fabio paginated-report create` sends), fabio's request likely matches the contract and the failure is a **capacity/region/feature-availability limitation** (paginated-report create-with-definition appears not enabled on this capacity), NOT a fabio bug. **Do not change the create body blind** — it cannot be validated on this tenant. The `paginated_report_create_show_delete_lifecycle` E2E test fails here for the same server-side reason. Possible robustness follow-up (unverifiable here): default the part path to `<displayName>.rdl` instead of the file basename. The `export` action does NOT depend on create and is validated independently (dry-run + live `PowerBIEntityNotFound` plumbing).
