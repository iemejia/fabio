...
- **add-datasource response is PascalCase (live-confirmed)**: The staging `POST .../datasources` response object uses PascalCase keys (`FabricItemType`, `Id`, `DisplayName`, `ItemReference`) — unlike the camelCase `fabricItemType` used in the REQUEST body and most other Fabric responses. Assertions/parsers over the add-datasource result must read `FabricItemType`, not `fabricItemType`.
