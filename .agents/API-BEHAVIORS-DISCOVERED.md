...
- **File-level status and commits**: `GET /workspaces/{ws}/git/status?includeFilesDetails=true` adds `fileChanges[]` to each item change. Each file has a root-relative forward-slash `path`, optional `workspaceChange`/`remoteChange`, and required `conflictType`. `POST .../git/commitToGit` supports `mode:"FileLevelSelective"` with `itemsWithFileSelection:[{objectId|logicalId,selectedFiles?}]`; `items` and `itemsWithFileSelection` are mutually exclusive. `selectedFiles:null` or `[]` commits all files for that item. Paths cannot be absolute, contain backslashes/wildcards/empty segments, or have segments ending in a dot or whitespace.
...
- **OneLake settings**: `GET /workspaces/{ws}/onelake/settings` returns tier, diagnostics, immutability, and `lifecycle.accessTimeTracking` (`Enabled`/`Disabled`). Modify via individual POST endpoints (`/modifyDefaultTier`, `/modifyAccessTimeTracking`, `/modifyDiagnostics`, `/modifyImmutabilityPolicy`).
- **OneLake access-time tracking**: `POST /workspaces/{ws}/onelake/settings/modifyAccessTimeTracking` with required body `{"status":"Enabled|Disabled"}` changes Azure Storage `LastAccessTime` tracking immediately. Reads and writes update the timestamp when enabled; metadata-only property, metadata, and tag operations do not. Requires workspace Admin + `OneLake.ReadWrite.All`; a workspace with no items can return `OneLakeNotProvisioned`.
...
- **Bulk operations (all LRO)**:
  - `POST /workspaces/{ws}/items/bulkExportDefinitions` — GA; no `beta` query parameter. Exports multiple item definitions. Body: `{"mode":"All"}` (all items) or `{"mode":"Selective","items":[{"id":"<uuid>"}]}`. Response: `{"itemDefinitionsIndex":[{"id":"...","rootPath":"...","displayName":"...","type":"..."}],"definitionParts":[{"path":"...","payload":"...","payloadType":"InlineBase64"}]}`. Only exports items caller has read+write permissions for. Items with protected sensitivity labels are excluded.
  - `POST /workspaces/{ws}/items/bulkImportDefinitions` — GA; no `beta` query parameter. Imports multiple item definitions. Body: `{"definitionParts":[{"path":"...","payload":"...","payloadType":"InlineBase64"}],"options":{"allowPairingByName":true}}`. The `allowPairingByName` option matches items by display name instead of logicalId (useful for initial clones).
  - `POST /workspaces/{ws}/items/bulkMove` — GA; moves multiple items between folders. Child items move with their parent and cannot be moved separately. Maximum 50 items per request.
...
- **Move-to-folder is GA**: The Preview designation was removed from both single-item `.../items/{itemId}/move` and `.../items/bulkMove`; paths and request bodies are unchanged. A parent and its child items move together, and a child cannot be moved independently.
...
- **Custom live pool settings**: Published/staging Spark compute now returns `customLivePoolSupport` (`Enabled`/`Disabled`) and `customLivePoolSettings` (`maxClustersToHydrate`, `clusterIdleTimeout`, `customLivePoolLifespan`). Update accepts both fields; omitting leaves them unchanged, `Disabled` retains hydration settings, and `customLivePoolSettings:null` removes them. `maxClustersToHydrate` has minimum 1 and a capacity-dependent maximum exposed read-only as `instancePool.maxClustersToHydrateLimit`; idle timeout is PT20M-PT24H and lifespan PT30M-PT24H.
...
- **Gateway member-count fields**: `numberOfMemberGateways`, `minMemberGatewayCount`, and `maxMemberGatewayCount` accept 1-11 (increased from 1-9). The fixed count is mutually exclusive with the min/max pair; both min and max are required together and max must be at least min. Default fixed count in the CLI is 1. Not applicable to `StreamingVirtualNetwork` gateways.
...
- **Delegated tenant-setting override APIs are GA**: The Preview designation was removed from tenant-setting update and capacity/domain/workspace delegated-override list/update/delete operations. Existing paths and rate limits are unchanged (25 requests/minute generally; listing one capacity's overrides allows 100/minute).
