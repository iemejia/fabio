...
The blog is provided by the [`starlight-blog`](https://starlight-blog-docs.vercel.app) plugin (registered
in `astro.config.mjs`). Posts are authored Markdown files in `src/content/docs/blog/`, served at
`/blog/` (index), `/blog/<slug>/`, `/blog/tags/<tag>/`, and `blog/rss.xml`.

1. Create `src/content/docs/blog/<slug>.md` with frontmatter:

   ```yaml
   ---
   title: Your post title
   date: 2026-08-11          # a valid YAML date
   authors: ismael          # an id from the `authors` map in astro.config.mjs
   excerpt: One-line summary shown in the index and tag pages.
   tags:
     - announcement
   ---
   ```

2. Use **relative** links to other pages (e.g. `../../getting-started/`), same rule as any page.
3. To add an author, extend the `authors` map in `astro.config.mjs` (`{ name, title?, url?, picture? }`).
4. The `/blog/` and `/blog/tags/` index routes are generated by the plugin (no authored file backs them);
   `check-links.mjs` recognizes them automatically once at least one post exists.
